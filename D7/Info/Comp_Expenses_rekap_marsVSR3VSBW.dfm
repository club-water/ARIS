object Comp_Expenses_rekap_marsVSR3VSBW_Frm: TComp_Expenses_rekap_marsVSR3VSBW_Frm
  Left = 180
  Top = 61
  Width = 1130
  Height = 610
  Caption = 'Comp_Expenses_rekap_marsVSR3VSBW_Frm'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1114
    Height = 45
    Align = alTop
    TabOrder = 0
    object Label3: TLabel
      Left = 4
      Top = 2
      Width = 48
      Height = 13
      Caption = 'Periode:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object Label4: TLabel
      Left = 833
      Top = 24
      Width = 32
      Height = 13
      Caption = 'Label4'
      Visible = False
    end
    object Label5: TLabel
      Left = 752
      Top = 24
      Width = 76
      Height = 13
      Caption = 'Loading Time  : '
      Visible = False
    end
    object Label6: TLabel
      Left = 895
      Top = 5
      Width = 32
      Height = 13
      Caption = 'Label6'
      Visible = False
    end
    object Label7: TLabel
      Left = 752
      Top = 6
      Width = 32
      Height = 13
      Caption = 'Label7'
      Visible = False
    end
    object Label2: TLabel
      Left = 107
      Top = 16
      Width = 31
      Height = 20
      Caption = 'S.D'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object BitBtn1: TBitBtn
      Left = 255
      Top = 5
      Width = 41
      Height = 38
      Hint = 'Refresh'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        1F7C1F7C1F7C}
    end
    object BitBtn2: TBitBtn
      Left = 301
      Top = 5
      Width = 41
      Height = 38
      Hint = 'Close'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        1F7C1F7C1F7C}
    end
    object Button9: TButton
      Left = 920
      Top = 24
      Width = 75
      Height = 25
      Caption = 'Button9'
      TabOrder = 2
      Visible = False
    end
    object LMDButton1: TLMDButton
      Left = 796
      Top = 2
      Width = 128
      Height = 29
      Caption = 'Generated Report'
      PopupMenu = PopupMenu1
      TabOrder = 3
      Visible = False
      ButtonLayout.Arrow = True
      DropDownMenu = PopupMenu1
    end
    object FromDate: TwwDBDateTimePicker
      Left = 9
      Top = 18
      Width = 95
      Height = 19
      BorderStyle = bsNone
      CalendarAttributes.Font.Charset = DEFAULT_CHARSET
      CalendarAttributes.Font.Color = clWindowText
      CalendarAttributes.Font.Height = -11
      CalendarAttributes.Font.Name = 'MS Sans Serif'
      CalendarAttributes.Font.Style = []
      CalendarAttributes.PopupYearOptions.StartYear = 2000
      Color = clBtnFace
      Epoch = 1950
      ButtonEffects.Transparent = True
      Frame.Enabled = True
      Frame.Transparent = True
      Frame.NonFocusBorders = []
      ShowButton = True
      TabOrder = 4
    end
    object ToDate: TwwDBDateTimePicker
      Left = 142
      Top = 16
      Width = 97
      Height = 19
      BorderStyle = bsNone
      CalendarAttributes.Font.Charset = DEFAULT_CHARSET
      CalendarAttributes.Font.Color = clWindowText
      CalendarAttributes.Font.Height = -11
      CalendarAttributes.Font.Name = 'MS Sans Serif'
      CalendarAttributes.Font.Style = []
      CalendarAttributes.PopupYearOptions.StartYear = 2000
      Color = clBtnFace
      Epoch = 1950
      ButtonEffects.Transparent = True
      Frame.Enabled = True
      Frame.Transparent = True
      Frame.NonFocusBorders = []
      ShowButton = True
      TabOrder = 5
    end
  end
  object Filter: TPageControl
    Left = 0
    Top = 45
    Width = 1114
    Height = 527
    ActivePage = TabSheet2
    Align = alClient
    TabOrder = 1
    object TabSheet2: TTabSheet
      Caption = 'Grids'
      ImageIndex = 2
      object cxGrid3: TcxGrid
        Left = 0
        Top = 41
        Width = 1106
        Height = 458
        Align = alClient
        TabOrder = 0
        object cxGrid3DBTableView1: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          OnCustomDrawCell = cxGrid3DBTableView1CustomDrawCell
          DataController.DataSource = UniDataSource1
          DataController.Summary.DefaultGroupSummaryItems = <
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'GROSS'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'GROSSKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'GROSSTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'JML'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'JMLKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'JMLTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NETTO'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NETTOKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NETTOTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NILAIDISCOUNT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NILAIDISCOUNTKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NILAIDISCOUNTTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'QTY'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'QTYKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'QTYTUNAI'
            end>
          DataController.Summary.FooterSummaryItems = <
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'mrs_nilai'
              Column = cxGrid3DBTableView1mrs_nilai
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'R3_amount'
              Column = cxGrid3DBTableView1R3_amount
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'selisih_mrs_sapbw'
              Column = cxGrid3DBTableView1selisih_mrs_sapbw
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'selisih_mrs_sapr3'
              Column = cxGrid3DBTableView1selisih_mrs_sapr3
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'BW_amount'
              Column = cxGrid3DBTableView1BW_amount
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'selisih_SAPR3_sapbw'
              Column = cxGrid3DBTableView1selisih_SAPR3_sapbw
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETVAT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'RTNGROSSAMT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'RTNVAT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'SLSDISCOUNT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'SLSGROSSAMT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'SLSVAT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTO'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTOKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTOTUNAI'
            end>
          DataController.Summary.SummaryGroups = <>
          OptionsBehavior.IncSearch = True
          OptionsCustomize.ColumnsQuickCustomization = True
          OptionsData.Deleting = False
          OptionsData.DeletingConfirmation = False
          OptionsData.Editing = False
          OptionsData.Inserting = False
          OptionsView.Footer = True
          OptionsView.FooterMultiSummaries = True
          OptionsView.GroupFooterMultiSummaries = True
          OptionsView.GroupFooters = gfAlwaysVisible
          Styles.ContentEven = _DataModule.cxStyle1
          Styles.ContentOdd = _DataModule.cxStyle3
          object cxGrid3DBTableView1mrs_SALESPOINTCD: TcxGridDBColumn
            DataBinding.FieldName = 'mrs_SALESPOINTCD'
            Styles.Content = _DataModule.cxStyle9
            Width = 127
          end
          object cxGrid3DBTableView1mrs_STOCKPOINTNAME: TcxGridDBColumn
            DataBinding.FieldName = 'mrs_STOCKPOINTNAME'
            Styles.Content = _DataModule.cxStyle9
            Width = 127
          end
          object cxGrid3DBTableView1mrs_TGL: TcxGridDBColumn
            DataBinding.FieldName = 'mrs_TGL'
            Styles.Content = _DataModule.cxStyle9
          end
          object cxGrid3DBTableView1mrs_nilai: TcxGridDBColumn
            DataBinding.FieldName = 'mrs_nilai'
            Styles.Content = _DataModule.cxStyle9
          end
          object cxGrid3DBTableView1BW_DocumentDate: TcxGridDBColumn
            DataBinding.FieldName = 'BW_DocumentDate'
          end
          object cxGrid3DBTableView1BW_StockPoint: TcxGridDBColumn
            DataBinding.FieldName = 'BW_StockPoint'
            Width = 81
          end
          object cxGrid3DBTableView1BW_amount: TcxGridDBColumn
            DataBinding.FieldName = 'BW_amount'
          end
          object cxGrid3DBTableView1R3_Document_Date: TcxGridDBColumn
            DataBinding.FieldName = 'R3_Document_Date'
            Styles.Content = _DataModule.cxStyle9
          end
          object cxGrid3DBTableView1R3_amount: TcxGridDBColumn
            DataBinding.FieldName = 'R3_amount'
            Styles.Content = _DataModule.cxStyle9
          end
          object cxGrid3DBTableView1R3_kode: TcxGridDBColumn
            DataBinding.FieldName = 'R3_kode'
            Styles.Content = _DataModule.cxStyle9
          end
          object cxGrid3DBTableView1R3_STOCKPOINTNAME: TcxGridDBColumn
            DataBinding.FieldName = 'R3_STOCKPOINTNAME'
            Styles.Content = _DataModule.cxStyle9
            Width = 142
          end
          object cxGrid3DBTableView1selisih_mrs_sapbw: TcxGridDBColumn
            Caption = 'MRS VS BW'
            DataBinding.FieldName = 'selisih_mrs_sapbw'
            Width = 107
          end
          object cxGrid3DBTableView1selisih_mrs_sapr3: TcxGridDBColumn
            Caption = 'MRS VS R3'
            DataBinding.FieldName = 'selisih_mrs_sapr3'
            Width = 122
          end
          object cxGrid3DBTableView1selisih_SAPR3_sapbw: TcxGridDBColumn
            Caption = 'R3 VS BW'
            DataBinding.FieldName = 'selisih_SAPR3_sapbw'
          end
          object cxGrid3DBTableView1Flag_Selisih: TcxGridDBColumn
            DataBinding.FieldName = 'Flag_Selisih'
          end
        end
        object cxGrid3Level1: TcxGridLevel
          GridView = cxGrid3DBTableView1
        end
      end
      object Panel3: TPanel
        Left = 0
        Top = 0
        Width = 1106
        Height = 41
        Align = alTop
        TabOrder = 1
        object LMDButton15: TLMDButton
          Left = 6
          Top = 5
          Width = 117
          Height = 30
          Caption = 'Export2Excell'
          TabOrder = 0
          OnClick = LMDButton15Click
          Glyph.Data = {
            
            1F7C1F7C1F7C}
        end
        object Button1: TButton
          Left = 128
          Top = 6
          Width = 117
          Height = 30
          Caption = 'Auto Width'
          TabOrder = 1
          OnClick = Button1Click
        end
      end
    end
    object TabSheet1: TTabSheet
      Caption = 'Detil'
      ImageIndex = 1
      object Splitter1: TSplitter
        Left = 545
        Top = 41
        Height = 458
        Visible = False
      end
      object Splitter2: TSplitter
        Left = 241
        Top = 41
        Height = 458
      end
      object Splitter3: TSplitter
        Left = 1103
        Top = 41
        Height = 458
        Align = alRight
      end
      object cxGrid1: TcxGrid
        Left = 0
        Top = 41
        Width = 241
        Height = 458
        Align = alLeft
        TabOrder = 0
        object cxGridDBTableView1: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          OnCustomDrawCell = cxGrid3DBTableView1CustomDrawCell
          DataController.DataSource = UniDataSource2
          DataController.Summary.DefaultGroupSummaryItems = <
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'GROSS'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'GROSSKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'GROSSTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'JML'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'JMLKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'JMLTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NETTO'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NETTOKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NETTOTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NILAIDISCOUNT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NILAIDISCOUNTKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NILAIDISCOUNTTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'QTY'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'QTYKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'QTYTUNAI'
            end>
          DataController.Summary.FooterSummaryItems = <
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'Amount'
              Column = cxGridDBTableView1Amount
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'NDGROSSAMT'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'NETGROSSAMT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETRTN'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETSLS'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTOTAL'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETVAT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'RTNGROSSAMT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'RTNVAT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'SLSDISCOUNT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'SLSGROSSAMT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'SLSVAT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTO'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTOKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTOTUNAI'
            end>
          DataController.Summary.SummaryGroups = <>
          OptionsBehavior.IncSearch = True
          OptionsCustomize.ColumnsQuickCustomization = True
          OptionsData.Deleting = False
          OptionsData.DeletingConfirmation = False
          OptionsData.Editing = False
          OptionsData.Inserting = False
          OptionsView.Footer = True
          OptionsView.FooterMultiSummaries = True
          OptionsView.GroupFooterMultiSummaries = True
          OptionsView.GroupFooters = gfAlwaysVisible
          Styles.ContentEven = _DataModule.cxStyle1
          Styles.ContentOdd = _DataModule.cxStyle3
          object cxGridDBTableView1doc_Date: TcxGridDBColumn
            DataBinding.FieldName = 'doc_Date'
            Width = 58
          end
          object cxGridDBTableView1Doc_No: TcxGridDBColumn
            DataBinding.FieldName = 'Doc_No'
            Width = 45
          end
          object cxGridDBTableView1StockPoint_KD: TcxGridDBColumn
            DataBinding.FieldName = 'StockPoint_KD'
            Width = 78
          end
          object cxGridDBTableView1STOCKPOINTNAME: TcxGridDBColumn
            DataBinding.FieldName = 'STOCKPOINTNAME'
            Width = 105
          end
          object cxGridDBTableView1Exp_type: TcxGridDBColumn
            DataBinding.FieldName = 'Exp_type'
            Width = 49
          end
          object cxGridDBTableView1description: TcxGridDBColumn
            DataBinding.FieldName = 'description'
            Width = 125
          end
          object cxGridDBTableView1Amount: TcxGridDBColumn
            DataBinding.FieldName = 'Amount'
          end
          object cxGridDBTableView1Vehicle_No: TcxGridDBColumn
            DataBinding.FieldName = 'Vehicle_No'
            Width = 64
          end
        end
        object cxGridLevel1: TcxGridLevel
          GridView = cxGridDBTableView1
        end
      end
      object cxGrid2: TcxGrid
        Left = 244
        Top = 41
        Width = 301
        Height = 458
        Align = alLeft
        TabOrder = 1
        object cxGridDBTableView2: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          OnCustomDrawCell = cxGrid3DBTableView1CustomDrawCell
          DataController.DataSource = UniDataSource3
          DataController.Summary.DefaultGroupSummaryItems = <
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'GROSS'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'GROSSKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'GROSSTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'JML'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'JMLKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'JMLTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NETTO'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NETTOKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NETTOTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NILAIDISCOUNT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NILAIDISCOUNTKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NILAIDISCOUNTTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'QTY'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'QTYKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'QTYTUNAI'
            end>
          DataController.Summary.FooterSummaryItems = <
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'Amount'
              Column = cxGridDBTableView2Amount
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'NDGROSSAMT'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'NETGROSSAMT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETRTN'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETSLS'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTOTAL'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETVAT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'RTNGROSSAMT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'RTNVAT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'SLSDISCOUNT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'SLSGROSSAMT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'SLSVAT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTO'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTOKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTOTUNAI'
            end>
          DataController.Summary.SummaryGroups = <>
          OptionsBehavior.IncSearch = True
          OptionsCustomize.ColumnsQuickCustomization = True
          OptionsData.Deleting = False
          OptionsData.DeletingConfirmation = False
          OptionsData.Editing = False
          OptionsData.Inserting = False
          OptionsView.Footer = True
          OptionsView.FooterMultiSummaries = True
          OptionsView.GroupFooterMultiSummaries = True
          OptionsView.GroupFooters = gfAlwaysVisible
          Styles.ContentEven = _DataModule.cxStyle1
          Styles.ContentOdd = _DataModule.cxStyle3
          object cxGridDBTableView2Kode: TcxGridDBColumn
            DataBinding.FieldName = 'Kode'
            Width = 46
          end
          object cxGridDBTableView2STOCKPOINTNAME: TcxGridDBColumn
            DataBinding.FieldName = 'STOCKPOINTNAME'
            Width = 122
          end
          object cxGridDBTableView2Assignment: TcxGridDBColumn
            DataBinding.FieldName = 'Assignment'
            Width = 132
          end
          object cxGridDBTableView2Document_Date: TcxGridDBColumn
            DataBinding.FieldName = 'Document_Date'
          end
          object cxGridDBTableView2Posting_Date: TcxGridDBColumn
            DataBinding.FieldName = 'Posting_Date'
          end
          object cxGridDBTableView2Document_Number: TcxGridDBColumn
            DataBinding.FieldName = 'Document_Number'
            Width = 114
          end
          object cxGridDBTableView2Amount: TcxGridDBColumn
            DataBinding.FieldName = 'Amount'
          end
          object cxGridDBTableView2Keterangan: TcxGridDBColumn
            DataBinding.FieldName = 'Keterangan'
            Width = 346
          end
          object cxGridDBTableView2UserName: TcxGridDBColumn
            DataBinding.FieldName = 'UserName'
            Width = 76
          end
          object cxGridDBTableView2CostCenter: TcxGridDBColumn
            DataBinding.FieldName = 'CostCenter'
            Width = 65
          end
          object cxGridDBTableView2Accountcode: TcxGridDBColumn
            DataBinding.FieldName = 'Accountcode'
            Width = 69
          end
        end
        object cxGridLevel2: TcxGridLevel
          GridView = cxGridDBTableView2
        end
      end
      object Panel12: TPanel
        Left = 0
        Top = 0
        Width = 1106
        Height = 41
        Align = alTop
        BevelOuter = bvNone
        Color = 16247003
        TabOrder = 2
        object RadioButton1: TRadioButton
          Left = 16
          Top = 8
          Width = 113
          Height = 17
          Caption = 'MARS VS SAP R3'
          Checked = True
          TabOrder = 0
          TabStop = True
          OnClick = RadioButton1Click
        end
        object RadioButton2: TRadioButton
          Left = 144
          Top = 8
          Width = 113
          Height = 17
          Caption = 'MARS VS SAP BW'
          TabOrder = 1
          OnClick = RadioButton2Click
        end
        object RadioButton3: TRadioButton
          Left = 288
          Top = 8
          Width = 137
          Height = 17
          Caption = 'SAP R3 VS SAP BW'
          TabOrder = 2
          OnClick = RadioButton3Click
        end
      end
      object cxGrid4: TcxGrid
        Left = 548
        Top = 41
        Width = 555
        Height = 458
        Align = alClient
        TabOrder = 3
        Visible = False
        object cxGridDBTableView3: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          OnCustomDrawCell = cxGrid3DBTableView1CustomDrawCell
          DataController.DataSource = UniDataSource4
          DataController.Summary.DefaultGroupSummaryItems = <
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'GROSS'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'GROSSKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'GROSSTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'JML'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'JMLKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'JMLTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NETTO'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NETTOKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NETTOTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NILAIDISCOUNT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NILAIDISCOUNTKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'NILAIDISCOUNTTUNAI'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'QTY'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'QTYKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              Position = spFooter
              FieldName = 'QTYTUNAI'
            end>
          DataController.Summary.FooterSummaryItems = <
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'BW_amount'
              Column = cxGridDBTableView3BW_amount
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'NDGROSSAMT'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'NETGROSSAMT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETRTN'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETSLS'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTOTAL'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETVAT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'RTNGROSSAMT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'RTNVAT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'SLSDISCOUNT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'SLSGROSSAMT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'SLSVAT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTO'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTOKREDIT'
            end
            item
              Format = '#,##0.00;(#,##0.00)'
              Kind = skSum
              FieldName = 'NETTOTUNAI'
            end>
          DataController.Summary.SummaryGroups = <>
          OptionsBehavior.IncSearch = True
          OptionsCustomize.ColumnsQuickCustomization = True
          OptionsData.Deleting = False
          OptionsData.DeletingConfirmation = False
          OptionsData.Editing = False
          OptionsData.Inserting = False
          OptionsView.Footer = True
          OptionsView.FooterMultiSummaries = True
          OptionsView.GroupFooterMultiSummaries = True
          OptionsView.GroupFooters = gfAlwaysVisible
          Styles.ContentEven = _DataModule.cxStyle1
          Styles.ContentOdd = _DataModule.cxStyle3
          object cxGridDBTableView3BW_StockPoint: TcxGridDBColumn
            DataBinding.FieldName = 'BW_StockPoint'
            Width = 81
          end
          object cxGridDBTableView3BW_Expensetype: TcxGridDBColumn
            DataBinding.FieldName = 'BW_Expensetype'
            Width = 153
          end
          object cxGridDBTableView3BW_amount: TcxGridDBColumn
            DataBinding.FieldName = 'BW_amount'
          end
          object cxGridDBTableView3BW_Account: TcxGridDBColumn
            DataBinding.FieldName = 'BW_Account'
            Width = 111
          end
          object cxGridDBTableView3BW_Branch: TcxGridDBColumn
            DataBinding.FieldName = 'BW_Branch'
            Width = 141
          end
          object cxGridDBTableView3BW_Company: TcxGridDBColumn
            DataBinding.FieldName = 'BW_Company'
            Width = 110
          end
          object cxGridDBTableView3BW_DocNo: TcxGridDBColumn
            DataBinding.FieldName = 'BW_DocNo'
            Width = 63
          end
          object cxGridDBTableView3BW_DocumentDate: TcxGridDBColumn
            DataBinding.FieldName = 'BW_DocumentDate'
          end
          object cxGridDBTableView3BW_NoPolisi: TcxGridDBColumn
            DataBinding.FieldName = 'BW_NoPolisi'
            Width = 67
          end
          object cxGridDBTableView3BW_PersonName: TcxGridDBColumn
            DataBinding.FieldName = 'BW_PersonName'
            Width = 98
          end
          object cxGridDBTableView3BW_PersonType: TcxGridDBColumn
            DataBinding.FieldName = 'BW_PersonType'
            Width = 86
          end
          object cxGridDBTableView3BW_SiteCanvas: TcxGridDBColumn
            DataBinding.FieldName = 'BW_SiteCanvas'
            Width = 175
          end
        end
        object cxGridLevel3: TcxGridLevel
          GridView = cxGridDBTableView3
        end
      end
    end
  end
  object FieldsImages: TImageList
    Left = 448
    Top = 244
    Bitmap = {
      
      000000000000}
  end
  object Daily_DB: TUniQuery
    Connection = _DataModule.UniConnection_Mars
    SQL.Strings = (
      'use MARS'
      '/****** Script for SelectTopNRows command from SSMS  ******/'
      
        'select t.* , case  WHEN ABS(abs(selisih_mrs_sapr3+selisih_mrs_sa' +
        'pbw)+selisih_SAPR3_sapbw) <> 0 THEN 1 '
      '         ELSE 0 end as Flag_Selisih '
      ' from ('
      'SELECT a.*,c.*,d.*'
      
        '      ,abs(isnull(a.mrs_nilai,0)-isnull(c.R3_amount,0)) as selis' +
        'ih_mrs_sapr3'
      
        '      ,abs(isnull(a.mrs_nilai,0)-isnull(d.bw_amount,0)) as selis' +
        'ih_mrs_sapbw'
      
        '      ,abs(isnull(c.R3_amount,0)-isnull(d.bw_amount,0)) as selis' +
        'ih_SAPR3_sapbw'
      '  FROM [MARS].[dbo].[VExpenses_Mrs_rekap] a  Full  join'
      
        '  mars.dbo.VExpenses_SAPR3_Rekap c on a.mrs_SALESPOINTCD=c.R3_ko' +
        'de and a.mrs_TGL=c.R3_Document_Date Full  join'
      
        '  mars.dbo.VExpenses_sapBW_rekap D ON D.bw_StockPoint=isnull(a.m' +
        'rs_STOCKPOINTNAME,C.R3_STOCKPOINTNAME) '
      
        '           and d.bw_DocumentDate=isnull(a.mrs_TGL,c.R3_Document_' +
        'Date)'
      
        '  where isnull(a.mrs_TGL,isnull(c.R3_Document_Date,d.bw_Document' +
        'Date))>=:0'
      
        '     and isnull(a.mrs_TGL,isnull(c.R3_Document_Date,d.bw_Documen' +
        'tDate))<=:1 '
      ' ) T')
    Left = 323
    Top = 166
    ParamData = <
      item
        DataType = ftDate
        Name = '0'
        ParamType = ptInput
        Value = 36526d
      end
      item
        DataType = ftDate
        Name = '1'
        ParamType = ptInput
        Value = 42005d
      end>
    object Daily_DBmrs_SALESPOINTCD: TStringField
      FieldName = 'mrs_SALESPOINTCD'
      Size = 50
    end
    object Daily_DBmrs_STOCKPOINTNAME: TStringField
      FieldName = 'mrs_STOCKPOINTNAME'
      ReadOnly = True
      Size = 250
    end
    object Daily_DBmrs_TGL: TDateTimeField
      FieldName = 'mrs_TGL'
    end
    object Daily_DBmrs_nilai: TFloatField
      FieldName = 'mrs_nilai'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBR3_Document_Date: TDateTimeField
      FieldName = 'R3_Document_Date'
      ReadOnly = True
    end
    object Daily_DBR3_amount: TFloatField
      FieldName = 'R3_amount'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBR3_kode: TIntegerField
      FieldName = 'R3_kode'
      ReadOnly = True
    end
    object Daily_DBR3_STOCKPOINTNAME: TStringField
      FieldName = 'R3_STOCKPOINTNAME'
      ReadOnly = True
      Size = 250
    end
    object Daily_DBselisih_mrs_sapr3: TFloatField
      FieldName = 'selisih_mrs_sapr3'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBselisih_mrs_sapbw: TFloatField
      FieldName = 'selisih_mrs_sapbw'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBFlag_Selisih: TIntegerField
      FieldName = 'Flag_Selisih'
      ReadOnly = True
      Required = True
    end
    object Daily_DBBW_DocumentDate: TDateTimeField
      FieldName = 'BW_DocumentDate'
      ReadOnly = True
    end
    object Daily_DBBW_StockPoint: TStringField
      FieldName = 'BW_StockPoint'
      ReadOnly = True
      Size = 250
    end
    object Daily_DBBW_amount: TFloatField
      FieldName = 'BW_amount'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBselisih_SAPR3_sapbw: TFloatField
      FieldName = 'selisih_SAPR3_sapbw'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
  end
  object DataSource1: TDataSource
    Left = 516
    Top = 281
  end
  object OpenDialog4: TOpenDialog
    Filter = 'All Report File|*.rtm'
    Left = 176
    Top = 320
  end
  object UniDataSource1: TUniDataSource
    DataSet = Daily_DB
    Left = 284
    Top = 169
  end
  object PopupMenu1: TPopupMenu
    AutoHotkeys = maManual
    Left = 328
    Top = 233
    object MenuItem1: TMenuItem
      Caption = 'Pivot'
    end
    object MenuItem2: TMenuItem
      Caption = 'Fast Report'
    end
  end
  object PopupMenu2: TPopupMenu
    AutoHotkeys = maManual
    Left = 228
    Top = 226
    object perBagian1: TMenuItem
      Caption = 'Per Bagian'
    end
    object PerOrang1: TMenuItem
      Caption = 'Per Orang'
    end
  end
  object UniDataSource2: TUniDataSource
    DataSet = Mrs
    Left = 508
    Top = 153
  end
  object Mrs: TUniQuery
    Connection = _DataModule.UniConnection_Mars
    SQL.Strings = (
      'use MARS'
      '/****** Script for SelectTopNRows command from SSMS  ******/'
      'SELECT a.*'
      '  FROM [MARS].[dbo].[VExpenses_Mrs]  a'
      '  where a.doc_Date>=:0'
      '     and a.doc_Date<=:1')
    Left = 587
    Top = 150
    ParamData = <
      item
        DataType = ftDate
        Name = '0'
        ParamType = ptInput
        Value = 36526d
      end
      item
        DataType = ftDate
        Name = '1'
        ParamType = ptInput
        Value = 42005d
      end>
    object Mrsdoc_Date: TDateTimeField
      FieldName = 'doc_Date'
    end
    object MrsDoc_No: TStringField
      FieldName = 'Doc_No'
      Size = 50
    end
    object Mrsreference_no: TStringField
      FieldName = 'reference_no'
      Size = 50
    end
    object MrsStockPoint_KD: TStringField
      FieldName = 'StockPoint_KD'
      Size = 50
    end
    object MrsRef_date: TDateTimeField
      FieldName = 'Ref_date'
    end
    object MrsPerson_KD: TStringField
      FieldName = 'Person_KD'
      Size = 50
    end
    object MrsPerson_name: TStringField
      FieldName = 'Person_name'
      Size = 250
    end
    object MrsExp_type: TStringField
      FieldName = 'Exp_type'
      Size = 50
    end
    object Mrsdescription: TStringField
      FieldName = 'description'
      Size = 250
    end
    object MrsAmount: TFloatField
      FieldName = 'Amount'
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object MrsCost_Center: TStringField
      FieldName = 'Cost_Center'
      Size = 50
    end
    object MrsSLND_No: TDateTimeField
      FieldName = 'SLND_No'
    end
    object Mrsunk: TStringField
      FieldName = 'unk'
      Size = 50
    end
    object MrsSLND_date: TDateTimeField
      FieldName = 'SLND_date'
    end
    object Mrssite: TStringField
      FieldName = 'site'
      Size = 50
    end
    object MrsVehicle_No: TStringField
      FieldName = 'Vehicle_No'
      Size = 50
    end
    object Mrsno_urut: TIntegerField
      FieldName = 'no_urut'
    end
    object MrsSTOCKPOINTNAME: TStringField
      FieldName = 'STOCKPOINTNAME'
      ReadOnly = True
      Size = 250
    end
  end
  object UniDataSource3: TUniDataSource
    DataSet = SAPR3
    Left = 508
    Top = 209
  end
  object SAPR3: TUniQuery
    Connection = _DataModule.UniConnection_Mars
    SQL.Strings = (
      'use MARS'
      '/****** Script for SelectTopNRows command from SSMS  ******/'
      'SELECT a.*'
      '  FROM [MARS].[dbo].[VExpenses_SAPR3]  a'
      '  where a.Document_Date>=:0'
      '     and a.Document_Date<=:1')
    Left = 595
    Top = 270
    ParamData = <
      item
        DataType = ftDate
        Name = '0'
        ParamType = ptInput
        Value = 36526d
      end
      item
        DataType = ftDate
        Name = '1'
        ParamType = ptInput
        Value = 42005d
      end>
    object SAPR3Assignment: TStringField
      DisplayLabel = 'R3_Assignment'
      FieldName = 'Assignment'
      Size = 250
    end
    object SAPR3Document_Date: TDateTimeField
      DisplayLabel = 'R3_Document_Date'
      FieldName = 'Document_Date'
    end
    object SAPR3Posting_Date: TDateTimeField
      DisplayLabel = 'R3_Posting_Date'
      FieldName = 'Posting_Date'
    end
    object SAPR3Document_Number: TStringField
      DisplayLabel = 'R3_Document_Number'
      FieldName = 'Document_Number'
      Size = 250
    end
    object SAPR3Amount: TFloatField
      DisplayLabel = 'R3_Amount'
      FieldName = 'Amount'
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object SAPR3Keterangan: TStringField
      DisplayLabel = 'R3_Keterangan'
      FieldName = 'Keterangan'
      Size = 250
    end
    object SAPR3UserName: TStringField
      DisplayLabel = 'R3_UserName'
      FieldName = 'UserName'
      Size = 250
    end
    object SAPR3CostCenter: TStringField
      DisplayLabel = 'R3_CostCenter'
      FieldName = 'CostCenter'
      Size = 250
    end
    object SAPR3Accountcode: TStringField
      DisplayLabel = 'R3_Accountcode'
      FieldName = 'Accountcode'
      Size = 250
    end
    object SAPR3Kode: TIntegerField
      DisplayLabel = 'R3_Kode'
      FieldName = 'Kode'
      ReadOnly = True
    end
    object SAPR3STOCKPOINTNAME: TStringField
      DisplayLabel = 'R3_STOCKPOINTNAME'
      FieldName = 'STOCKPOINTNAME'
      ReadOnly = True
      Size = 250
    end
  end
  object UniDataSource4: TUniDataSource
    DataSet = SAPBW
    Left = 508
    Top = 209
  end
  object SAPBW: TUniQuery
    Connection = _DataModule.UniConnection_Mars
    SQL.Strings = (
      'use MARS'
      '/****** Script for SelectTopNRows command from SSMS  ******/'
      'SELECT a.*'
      '  FROM [MARS].[dbo].[VExpenses_SAPBW]  a'
      '  where a.BW_DocumentDate>=:0'
      '     and a.BW_DocumentDate<=:1')
    Left = 587
    Top = 206
    ParamData = <
      item
        DataType = ftDate
        Name = '0'
        ParamType = ptInput
        Value = 36526d
      end
      item
        DataType = ftDate
        Name = '1'
        ParamType = ptInput
        Value = 42005d
      end>
    object SAPBWBW_Expensetype: TStringField
      FieldName = 'BW_Expensetype'
      Size = 250
    end
    object SAPBWBW_Account: TStringField
      FieldName = 'BW_Account'
      Size = 250
    end
    object SAPBWBW_Branch: TStringField
      FieldName = 'BW_Branch'
      Size = 250
    end
    object SAPBWBW_Company: TStringField
      FieldName = 'BW_Company'
      Size = 250
    end
    object SAPBWBW_DocNo: TStringField
      FieldName = 'BW_DocNo'
      Size = 250
    end
    object SAPBWBW_DocumentDate: TDateTimeField
      FieldName = 'BW_DocumentDate'
    end
    object SAPBWBW_NoPolisi: TStringField
      FieldName = 'BW_NoPolisi'
      Size = 250
    end
    object SAPBWBW_PersonName: TStringField
      FieldName = 'BW_PersonName'
      Size = 250
    end
    object SAPBWBW_PersonType: TStringField
      FieldName = 'BW_PersonType'
      Size = 250
    end
    object SAPBWBW_SiteCanvas: TStringField
      FieldName = 'BW_SiteCanvas'
      Size = 250
    end
    object SAPBWBW_StockPoint: TStringField
      FieldName = 'BW_StockPoint'
      Size = 250
    end
    object SAPBWBW_amount: TFloatField
      FieldName = 'BW_amount'
      DisplayFormat = '#,##0.;(#,##0.)'
    end
  end
end
