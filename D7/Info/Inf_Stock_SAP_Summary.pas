...
type
  TInf_Stock_SAP_SummaryFrm = class(TForm)
    Panel1: TPanel;
    Label3: TLabel;
    BitBtn1: TBitBtn;
    BitBtn2: TBitBtn;
    ...
    Daily_DBITEMCD: TStringField;
    Daily_DBMaterial_Nama: TStringField;
    Daily_DBSaldo_Qty: TIntegerField;
    ...
    cxGrid3DBTableView1ITEMCD: TcxGridDBColumn;
    cxGrid3DBTableView1Material_Nama: TcxGridDBColumn;
    cxGrid3DBTableView1Saldo_Qty: TcxGridDBColumn;
    TabSheet3: TTabSheet;
    Button2: TButton;
    ComboBox1: TComboBox;
...
var
  Inf_Stock_SAP_SummaryFrm: TInf_Stock_SAP_SummaryFrm;
implementation
...
procedure TInf_Stock_SAP_SummaryFrm.FormClose(Sender: TObject; var Action: TCloseAction);
var FileName :String;
begin
 ...
 if (not fcCube1.DataSet.IsEmpty) and (fcSlice1.HaveLayout) then
  ...
  Action := caFree;
  Inf_Stock_SAP_SummaryFrm := nil;
end;

procedure TInf_Stock_SAP_SummaryFrm.BitBtn1Click(Sender: TObject);
var AIndex: integer;
    Vmenit,Vdetik : integer;
    FileName :String;
begin
  cxGrid3DBTableView1.DataController.DataSource:=nil;
  FileName:=ExtractFileDir(Application.ExeName)+'\Inf_Stock_SAP_SummaryFrm.mds';
 if (not fcCube1.DataSet.IsEmpty) and (fcSlice1.HaveLayout) then
  begin
    fcSlice1.SaveToFile(FileName);
  end;
...
procedure TInf_Stock_SAP_SummaryFrm.BitBtn2Click(Sender: TObject);
begin
  close;
end;

procedure TInf_Stock_SAP_SummaryFrm.FormShow(Sender: TObject);
var i: Integer;
begin
  Daily_DB.SQL.Text:=Daily_DB.SQL.Text+_DataModule.List_akses_Branch_where('PLANTCD');
...
procedure TInf_Stock_SAP_SummaryFrm.Button7Click(Sender: TObject);
var i:integer;
    vlist:string;
begin
  fcCube1.Active := false;
...
procedure TInf_Stock_SAP_SummaryFrm.LMDButton15Click(Sender: TObject);
begin
...
procedure TInf_Stock_SAP_SummaryFrm.Button1Click(Sender: TObject);
begin
  cxGrid3DBTableView1.ApplyBestFit;
end;

procedure TInf_Stock_SAP_SummaryFrm.cxGrid3DBTableView1CustomDrawCell(
  Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
var
  val : String;
...
procedure TInf_Stock_SAP_SummaryFrm.LMDButton2Click(Sender: TObject);
begin
  cxGrid3DBTableView1.DataController.DataSource:=UniDataSource1;
end;

procedure TInf_Stock_SAP_SummaryFrm.Button2Click(Sender: TObject);
var j:integer;
begin
...
procedure TInf_Stock_SAP_SummaryFrm.SpeedButton1Click(Sender: TObject);
begin
  RichEdit1.Lines.Text:=RichEdit1.Lines.Text+' And '+ComboBox1.Text+' in ()';
end;

procedure TInf_Stock_SAP_SummaryFrm.SpeedButton2Click(Sender: TObject);
begin
  Daily_DB.SQL.Clear;
...
procedure TInf_Stock_SAP_SummaryFrm.TabSheet3Show(Sender: TObject);
begin
 Button2Click(Sender);
end;
