...
type
  TInf_Sales_rekap_QTY_marsFrm = class(TForm)
    Panel1: TPanel;
    Label3: TLabel;
    BitBtn1: TBitBtn;
    BitBtn2: TBitBtn;
...
var
  Inf_Sales_rekap_QTY_marsFrm: TInf_Sales_rekap_QTY_marsFrm;
implementation
...
procedure TInf_Sales_rekap_QTY_marsFrm.FormClose(Sender: TObject; var Action: TCloseAction);
var FileName :String;
begin
 ...
 if (not fcCube1.DataSet.IsEmpty) and (fcSlice1.HaveLayout) then
  ...
  Action := caFree;
  Inf_Sales_rekap_QTY_marsFrm := nil;
end;

procedure TInf_Sales_rekap_QTY_marsFrm.BitBtn1Click(Sender: TObject);
var AIndex: integer;
    Vmenit,Vdetik : integer;
    FileName :String;
begin

  FileName:=ExtractFileDir(Application.ExeName)+'\Inf_Sales_rekap_QTY_marsFrm.mds';
 if (not fcCube1.DataSet.IsEmpty) and (fcSlice1.HaveLayout) then
  begin
    fcSlice1.SaveToFile(FileName);
  end;
...
procedure TInf_Sales_rekap_QTY_marsFrm.BitBtn2Click(Sender: TObject);
begin
  close;
end;

procedure TInf_Sales_rekap_QTY_marsFrm.FormShow(Sender: TObject);
var i: Integer;
begin
...
procedure TInf_Sales_rekap_QTY_marsFrm.Button7Click(Sender: TObject);
var i:integer;
    vlist:string;
begin
  fcCube1.Active := false;
...
procedure TInf_Sales_rekap_QTY_marsFrm.LMDButton15Click(Sender: TObject);
begin
...
procedure TInf_Sales_rekap_QTY_marsFrm.Button1Click(Sender: TObject);
begin
  cxGrid3DBTableView1.ApplyBestFit;
end;

procedure TInf_Sales_rekap_QTY_marsFrm.cxGrid3DBTableView1CustomDrawCell(
  Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
var
  val : String;
