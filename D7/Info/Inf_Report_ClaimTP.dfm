object Inf_Report_ClaimTPFrm: TInf_Report_ClaimTPFrm
  Left = 271
  Top = 85
  Width = 1155
  Height = 608
  Caption = 'Inf_Report_ClaimTPFrm'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1139
    Height = 45
    Align = alTop
    TabOrder = 0
    object Label3: TLabel
      Left = 4
      Top = 2
      Width = 48
      Height = 13
      Caption = 'Periode:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object Label4: TLabel
      Left = 833
      Top = 24
      Width = 32
      Height = 13
      Caption = 'Label4'
      Visible = False
    end
    object Label5: TLabel
      Left = 752
      Top = 24
      Width = 76
      Height = 13
      Caption = 'Loading Time  : '
      Visible = False
    end
    object Label6: TLabel
      Left = 895
      Top = 5
      Width = 32
      Height = 13
      Caption = 'Label6'
      Visible = False
    end
    object Label7: TLabel
      Left = 752
      Top = 6
      Width = 32
      Height = 13
      Caption = 'Label7'
      Visible = False
    end
    object Label2: TLabel
      Left = 104
      Top = 18
      Width = 31
      Height = 20
      Caption = 'S.D'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object ToDate: TDateTimePicker
      Left = 137
      Top = 18
      Width = 101
      Height = 21
      Date = 40772.000000000000000000
      Time = 40772.000000000000000000
      TabOrder = 3
    end
    object FromDate: TDateTimePicker
      Left = 4
      Top = 18
      Width = 97
      Height = 21
      Date = 42370.000000000000000000
      Time = 42370.000000000000000000
      TabOrder = 0
    end
    object BitBtn1: TBitBtn
      Left = 243
      Top = 5
      Width = 41
      Height = 38
      Hint = 'Refresh'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        1F7C1F7C1F7C}
    end
    object BitBtn2: TBitBtn
      Left = 293
      Top = 5
      Width = 41
      Height = 38
      Hint = 'Close'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        1F7C1F7C1F7C}
    end
    object SpinEdit1: TSpinEdit
      Left = 510
      Top = 8
      Width = 121
      Height = 22
      MaxValue = 2020
      MinValue = 2017
      TabOrder = 4
      Value = 2018
      Visible = False
    end
  end
  object Filter: TPageControl
    Left = 0
    Top = 45
    Width = 1139
    Height = 524
    ActivePage = TabSheet2
    Align = alClient
    TabOrder = 1
    object TabSheet1: TTabSheet
      Caption = 'Pivot'
      object Panel2: TPanel
        Left = 0
        Top = 34
        Width = 1131
        Height = 24
        Align = alTop
        TabOrder = 0
        object fcToolBar1: TfcToolBar
          Left = 1
          Top = 1
          Width = 1129
          Height = 22
          Align = alClient
          AutoSize = True
          Caption = 'fcToolBar1'
          Color = clBtnFace
          EdgeBorders = []
          Flat = True
          ParentColor = False
          TabOrder = 0
          Grid = fcGrid1
        end
      end
      object fcGrid1: TfcGrid
        Left = 0
        Top = 58
        Width = 1131
        Height = 438
        DefaultColWidth = 100
        FieldsImages = FieldsImages
        Styles.CaptionArea.FillColor = 15466495
        Styles.CaptionArea.TextColor = clBlack
        Styles.CaptionArea.Font.Charset = DEFAULT_CHARSET
        Styles.CaptionArea.Font.Color = clBlack
        Styles.CaptionArea.Font.Height = -11
        Styles.CaptionArea.Font.Name = 'MS Sans Serif'
        Styles.CaptionArea.Font.Style = []
        Styles.HeaderArea.FillColor = clBtnFace
        Styles.HeaderArea.TextColor = clGray
        Styles.HeaderArea.Font.Charset = DEFAULT_CHARSET
        Styles.HeaderArea.Font.Color = clGray
        Styles.HeaderArea.Font.Height = -11
        Styles.HeaderArea.Font.Name = 'MS Sans Serif'
        Styles.HeaderArea.Font.Style = []
        Styles.DataArea.FillColor = clWhite
        Styles.DataArea.TextColor = clGray
        Styles.DataArea.Font.Charset = DEFAULT_CHARSET
        Styles.DataArea.Font.Color = clGray
        Styles.DataArea.Font.Height = -11
        Styles.DataArea.Font.Name = 'MS Sans Serif'
        Styles.DataArea.Font.Style = []
        Styles.ServiceArea.FillColor = 15466495
        Styles.ServiceArea.TextColor = clBlack
        Styles.ServiceArea.Font.Charset = DEFAULT_CHARSET
        Styles.ServiceArea.Font.Color = clBlack
        Styles.ServiceArea.Font.Height = -11
        Styles.ServiceArea.Font.Name = 'MS Sans Serif'
        Styles.ServiceArea.Font.Style = []
        Styles.ActiveItem.FillColor = clActiveCaption
        Styles.ActiveItem.TextColor = clCaptionText
        Styles.ActiveItem.Font.Charset = DEFAULT_CHARSET
        Styles.ActiveItem.Font.Color = clCaptionText
        Styles.ActiveItem.Font.Height = -11
        Styles.ActiveItem.Font.Name = 'MS Sans Serif'
        Styles.ActiveItem.Font.Style = []
        Styles.InactiveItem.FillColor = clInactiveCaption
        Styles.InactiveItem.TextColor = clInactiveCaptionText
        Styles.InactiveItem.Font.Charset = DEFAULT_CHARSET
        Styles.InactiveItem.Font.Color = clInactiveCaptionText
        Styles.InactiveItem.Font.Height = -11
        Styles.InactiveItem.Font.Name = 'MS Sans Serif'
        Styles.InactiveItem.Font.Style = []
        Styles.MeasureItem.FillColor = clMoneyGreen
        Styles.MeasureItem.TextColor = clCaptionText
        Styles.MeasureItem.Font.Charset = DEFAULT_CHARSET
        Styles.MeasureItem.Font.Color = clCaptionText
        Styles.MeasureItem.Font.Height = -11
        Styles.MeasureItem.Font.Name = 'MS Sans Serif'
        Styles.MeasureItem.Font.Style = []
        Styles.FieldsItem.FillColor = clMoneyGreen
        Styles.FieldsItem.TextColor = clCaptionText
        Styles.FieldsItem.Font.Charset = DEFAULT_CHARSET
        Styles.FieldsItem.Font.Color = clCaptionText
        Styles.FieldsItem.Font.Height = -11
        Styles.FieldsItem.Font.Name = 'MS Sans Serif'
        Styles.FieldsItem.Font.Style = []
        Styles.AxisCells.FillColor = clBtnFace
        Styles.AxisCells.TextColor = clBlack
        Styles.AxisCells.Font.Charset = DEFAULT_CHARSET
        Styles.AxisCells.Font.Color = clBlack
        Styles.AxisCells.Font.Height = -11
        Styles.AxisCells.Font.Name = 'MS Sans Serif'
        Styles.AxisCells.Font.Style = []
        Styles.AxisCellsSelected.FillColor = clBtnShadow
        Styles.AxisCellsSelected.TextColor = clBtnText
        Styles.AxisCellsSelected.Font.Charset = DEFAULT_CHARSET
        Styles.AxisCellsSelected.Font.Color = clBtnText
        Styles.AxisCellsSelected.Font.Height = -11
        Styles.AxisCellsSelected.Font.Name = 'MS Sans Serif'
        Styles.AxisCellsSelected.Font.Style = []
        Styles.DataCells.FillColor = clWhite
        Styles.DataCells.TextColor = clBlack
        Styles.DataCells.Font.Charset = DEFAULT_CHARSET
        Styles.DataCells.Font.Color = clBlack
        Styles.DataCells.Font.Height = -11
        Styles.DataCells.Font.Name = 'MS Sans Serif'
        Styles.DataCells.Font.Style = []
        Styles.DataCellsTotals.FillColor = 2595309
        Styles.DataCellsTotals.TextColor = clBlack
        Styles.DataCellsTotals.Font.Charset = DEFAULT_CHARSET
        Styles.DataCellsTotals.Font.Color = clBlack
        Styles.DataCellsTotals.Font.Height = -11
        Styles.DataCellsTotals.Font.Name = 'MS Sans Serif'
        Styles.DataCellsTotals.Font.Style = []
        Styles.DataCellsSelected.FillColor = clHighlight
        Styles.DataCellsSelected.TextColor = clHighlightText
        Styles.DataCellsSelected.Font.Charset = DEFAULT_CHARSET
        Styles.DataCellsSelected.Font.Color = clHighlightText
        Styles.DataCellsSelected.Font.Height = -11
        Styles.DataCellsSelected.Font.Name = 'MS Sans Serif'
        Styles.DataCellsSelected.Font.Style = []
        Styles.UseParentFont = False
        Slice = fcSlice1
        Align = alClient
        TabOrder = 1
      end
      object Panel3: TPanel
        Left = 0
        Top = 0
        Width = 1131
        Height = 34
        Align = alTop
        TabOrder = 2
        object LMDButton1: TLMDButton
          Left = 3
          Top = 2
          Width = 134
          Height = 29
          Caption = 'Connect To Pivot'
          TabOrder = 0
          OnClick = LMDButton1Click
        end
      end
    end
    object TabSheet2: TTabSheet
      Caption = 'Grids'
      ImageIndex = 2
      object Panel6: TPanel
        Left = 0
        Top = 0
        Width = 1131
        Height = 41
        Align = alTop
        TabOrder = 0
        object Label1: TLabel
          Left = 288
          Top = 4
          Width = 167
          Height = 13
          Caption = '1 : kesesuaian WBS dan No easap'
        end
        object Label8: TLabel
          Left = 288
          Top = 20
          Width = 195
          Height = 13
          Caption = '2 : kesesuaian No Easap dan Plant Code'
        end
        object Label9: TLabel
          Left = 504
          Top = 4
          Width = 199
          Height = 13
          Caption = '3 : Data Tidak Ditemukan Di table EASAP'
        end
        object Button3: TButton
          Left = 1014
          Top = 6
          Width = 75
          Height = 25
          Caption = 'Refresh Pivot'
          TabOrder = 0
          Visible = False
        end
        object Button5: TButton
          Left = 934
          Top = 6
          Width = 75
          Height = 25
          Caption = 'More Filter'
          TabOrder = 1
          Visible = False
        end
        object Button8: TButton
          Left = 852
          Top = 6
          Width = 75
          Height = 25
          Caption = 'Refresh Pivot'
          TabOrder = 2
          Visible = False
          OnClick = Button7Click
        end
        object LMDButton5: TLMDButton
          Left = 11
          Top = 4
          Width = 134
          Height = 29
          Caption = 'Connect To Grids'
          TabOrder = 3
          OnClick = LMDButton5Click
        end
        object LMDButton15: TLMDButton
          Left = 149
          Top = 4
          Width = 132
          Height = 28
          Caption = 'Export2Excell'
          PopupMenu = PopupMenu3
          TabOrder = 4
          ButtonLayout.Arrow = True
          Glyph.Data = {
            
            1F7C1F7C1F7C}
          DropDownMenu = PopupMenu3
        end
      end
      object cxGrid3: TcxGrid
        Left = 0
        Top = 41
        Width = 1131
        Height = 455
        Align = alClient
        TabOrder = 1
        object cxGrid3DBTableView1: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          DataController.DataSource = UniDataSource1
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <
            item
              Format = 'Doc : #,##0.;(#,##0.)'
              Kind = skCount
              FieldName = 'PERIODE_CYCLE'
              Column = cxGrid3DBTableView1PERIODE_CYCLE
            end
            item
              Format = '#,##0.;(#,##0.)'#9
              Kind = skSum
              FieldName = 'SALES_QTY_BASED_UOM'
              Column = cxGrid3DBTableView1SALES_QTY_BASED_UOM
            end
            item
              Format = '#,##0.;(#,##0.)'#9
              Kind = skSum
              FieldName = 'NOMINAL_KLAIM_IAP'
              Column = cxGrid3DBTableView1NOMINAL_KLAIM_IAP
            end
            item
              Format = '#,##0.;(#,##0.)'#9
              Kind = skSum
              FieldName = 'VALIDASI_TSP_FINAL'
              Column = cxGrid3DBTableView1VALIDASI_TSP_FINAL
            end
            item
              Format = '#,##0.;(#,##0.)'#9
              Kind = skSum
              FieldName = 'TOLAKAN'
              Column = cxGrid3DBTableView1TOLAKAN
            end
            item
              Format = '#,##0.;(#,##0.)'#9
              Kind = skSum
              FieldName = 'ACCRUED_LIST_JULI'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_LIST_JUNI'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_LIST_MARET'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_LIST_MEI'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_LIST_NOVEMBER'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_LIST_OKTOBER'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_LIST_SEPTEMBER'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'Ca_Acrual_agust'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'Ca_Acrual_apr'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'Ca_Acrual_des'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'Ca_Acrual_Feb'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'Ca_Acrual_Jan'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'Ca_Acrual_jul'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'Ca_Acrual_jun'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'Ca_Acrual_mar'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'Ca_Acrual_mei'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'Ca_Acrual_nov'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'Ca_Acrual_okt'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'Ca_Acrual_sept'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'REALISASI_GL_AGUSTUS'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'REALISASI_GL_APRIL'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'REALISASI_GL_DESEMBER'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'REALISASI_GL_FEBRUARI'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'REALISASI_GL_JANUARI'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'REALISASI_GL_JULI'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'REALISASI_GL_JUNI'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'REALISASI_GL_MARET'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'REALISASI_GL_MEI'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'REALISASI_GL_NOVEMBER'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'REALISASI_GL_OKTOBER'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'REALISASI_GL_SEPTEMBER'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'BTA'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_REVERSE_AGUSTUS'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_REVERSE_APRIL'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_REVERSE_DESEMBER'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_REVERSE_FEBRUARI'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_REVERSE_JANUARI'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_REVERSE_JUNI'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_REVERSE_MARET'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_REVERSE_MEI'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_REVERSE_NOVEMBER'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_REVERSE_OKTOBER'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_REVERSE_SEPTEMBER'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'ACCRUED_REVERSE_JULI'
            end>
          DataController.Summary.SummaryGroups = <>
          OptionsCustomize.ColumnsQuickCustomization = True
          OptionsData.Deleting = False
          OptionsData.Inserting = False
          OptionsView.Footer = True
          object cxGrid3DBTableView1Status_Check: TcxGridDBColumn
            DataBinding.FieldName = 'Status_Check'
            Width = 82
          end
          object cxGrid3DBTableView1kode_branch: TcxGridDBColumn
            DataBinding.FieldName = 'kode_branch'
            Styles.Content = _DataModule.cxStyle6
            Width = 114
          end
          object cxGrid3DBTableView1WBS: TcxGridDBColumn
            DataBinding.FieldName = 'WBS'
            Styles.Content = _DataModule.cxStyle6
            Width = 125
          end
          object cxGrid3DBTableView1EASAP: TcxGridDBColumn
            DataBinding.FieldName = 'EASAP'
            Styles.Content = _DataModule.cxStyle6
            Width = 96
          end
          object cxGrid3DBTableView1NO_TRANSAKSI: TcxGridDBColumn
            DataBinding.FieldName = 'NO_TRANSAKSI'
            Styles.Content = _DataModule.cxStyle6
            Width = 119
          end
          object cxGrid3DBTableView1Status: TcxGridDBColumn
            DataBinding.FieldName = 'Status'
            Visible = False
            Width = 61
          end
          object cxGrid3DBTableView1TGL_TRANSAKSI: TcxGridDBColumn
            DataBinding.FieldName = 'TGL_TRANSAKSI'
            Width = 127
          end
          object cxGrid3DBTableView1PERIODE_CYCLE: TcxGridDBColumn
            DataBinding.FieldName = 'PERIODE_CYCLE'
            Width = 103
          end
          object cxGrid3DBTableView1tahun_cycle: TcxGridDBColumn
            DataBinding.FieldName = 'tahun_cycle'
            Width = 103
          end
          object cxGrid3DBTableView1BRANCH: TcxGridDBColumn
            DataBinding.FieldName = 'BRANCH'
            Width = 222
          end
          object cxGrid3DBTableView1KODE_CAB_IAP: TcxGridDBColumn
            DataBinding.FieldName = 'KODE_CAB_IAP'
            Width = 102
          end
          object cxGrid3DBTableView1Lcabang_IAP: TcxGridDBColumn
            DataBinding.FieldName = 'Lcabang_IAP'
            Width = 102
          end
          object cxGrid3DBTableView1NO_SK: TcxGridDBColumn
            DataBinding.FieldName = 'NO_SK'
            Width = 109
          end
          object cxGrid3DBTableView1SUB_BRAND: TcxGridDBColumn
            DataBinding.FieldName = 'SUB_BRAND'
            Width = 92
          end
          object cxGrid3DBTableView1MATERIAL: TcxGridDBColumn
            DataBinding.FieldName = 'MATERIAL'
            Width = 150
          end
          object cxGrid3DBTableView1REF_IAP: TcxGridDBColumn
            DataBinding.FieldName = 'REF_IAP'
            Width = 195
          end
          object cxGrid3DBTableView1REF_TSP: TcxGridDBColumn
            DataBinding.FieldName = 'REF_TSP'
            Width = 167
          end
          object cxGrid3DBTableView1PILAR: TcxGridDBColumn
            DataBinding.FieldName = 'PILAR'
            Width = 101
          end
          object cxGrid3DBTableView1Grouping_name: TcxGridDBColumn
            DataBinding.FieldName = 'Grouping_name'
          end
          object cxGrid3DBTableView1GROUPING: TcxGridDBColumn
            DataBinding.FieldName = 'GROUPING'
            Width = 93
          end
          object cxGrid3DBTableView1JENIS: TcxGridDBColumn
            DataBinding.FieldName = 'JENIS'
            Width = 166
          end
          object cxGrid3DBTableView1SALES_QTY_BASED_UOM: TcxGridDBColumn
            DataBinding.FieldName = 'SALES_QTY_BASED_UOM'
            Width = 157
          end
          object cxGrid3DBTableView1NOMINAL_KLAIM_IAP: TcxGridDBColumn
            DataBinding.FieldName = 'NOMINAL_KLAIM_IAP'
            Width = 144
          end
          object cxGrid3DBTableView1VALIDASI_TSP_FINAL: TcxGridDBColumn
            DataBinding.FieldName = 'VALIDASI_TSP_FINAL'
            Width = 143
          end
          object cxGrid3DBTableView1TOLAKAN: TcxGridDBColumn
            DataBinding.FieldName = 'TOLAKAN'
            Width = 134
          end
          object cxGrid3DBTableView1STOCK_POINT: TcxGridDBColumn
            DataBinding.FieldName = 'STOCK_POINT'
            Visible = False
            Width = 231
          end
          object cxGrid3DBTableView1ORDER_DATE: TcxGridDBColumn
            DataBinding.FieldName = 'ORDER_DATE'
            Visible = False
            Width = 119
          end
          object cxGrid3DBTableView1INVOICE_NUMBER: TcxGridDBColumn
            DataBinding.FieldName = 'INVOICE_NUMBER'
            Visible = False
          end
          object cxGrid3DBTableView1CUSTOMER: TcxGridDBColumn
            DataBinding.FieldName = 'CUSTOMER'
            Visible = False
            Width = 266
          end
          object cxGrid3DBTableView1TYPE_OUTLET: TcxGridDBColumn
            DataBinding.FieldName = 'TYPE_OUTLET'
            Visible = False
          end
          object cxGrid3DBTableView1SALES_QTY_FRACTION: TcxGridDBColumn
            DataBinding.FieldName = 'SALES_QTY_FRACTION'
            Visible = False
            Width = 99
          end
          object cxGrid3DBTableView1VALIDASI_TSP_ON_PROGRESS: TcxGridDBColumn
            DataBinding.FieldName = 'VALIDASI_TSP_ON_PROGRESS'
            Visible = False
            Width = 127
          end
          object cxGrid3DBTableView1BELUM_VALIDASI: TcxGridDBColumn
            DataBinding.FieldName = 'BELUM_VALIDASI'
            Visible = False
            Width = 123
          end
          object cxGrid3DBTableView1TOLAKAN_BLM_VALIDASI: TcxGridDBColumn
            DataBinding.FieldName = 'TOLAKAN_BLM_VALIDASI'
            Visible = False
          end
          object cxGrid3DBTableView1TOLAKAN_SDH_VALIDASI: TcxGridDBColumn
            DataBinding.FieldName = 'TOLAKAN_SDH_VALIDASI'
            Visible = False
          end
          object cxGrid3DBTableView1CStatus: TcxGridDBColumn
            DataBinding.FieldName = 'CStatus'
          end
        end
        object cxGrid3Level1: TcxGridLevel
          GridView = cxGrid3DBTableView1
        end
      end
    end
    object TabSheet3: TTabSheet
      Caption = 'TabSheet3'
      ImageIndex = 2
      object Button4: TButton
        Left = 24
        Top = 8
        Width = 75
        Height = 25
        Caption = 'Master'
        TabOrder = 0
        OnClick = Button4Click
      end
      object Button7: TButton
        Left = 24
        Top = 40
        Width = 75
        Height = 25
        Caption = 'Import Master'
        TabOrder = 1
        OnClick = Button7Click
      end
      object Button9: TButton
        Left = 104
        Top = 40
        Width = 75
        Height = 25
        Caption = 'Import detil'
        TabOrder = 2
        OnClick = Button9Click
      end
      object Button6: TButton
        Left = 104
        Top = 8
        Width = 75
        Height = 25
        Caption = 'Detil'
        TabOrder = 3
        OnClick = Button6Click
      end
    end
  end
  object FieldsImages: TImageList
    Left = 600
    Top = 292
    Bitmap = {
      
      000000000000}
  end
  object fcCube1: TfcCube
    DataSet = Daily_DB
    Options = [mdcoMakeDates, mdcoLoadAllFields, mdcoRefreshOnDatasetReopen]
    DefaultFormat.BooleanFormat.FormatStr = 'False,True'
    DefaultFormat.BooleanFormat.Kind = fkBoolean
    DefaultFormat.DateFormat.FormatStr = 'dd/MM/yyyy'
    DefaultFormat.DateFormat.Kind = fkDateTime
    DefaultFormat.TimeFormat.FormatStr = 'hh:mm'
    DefaultFormat.TimeFormat.Kind = fkDateTime
    DefaultFormat.DateTimeFormat.FormatStr = 'dd/MM/yyyy'
    DefaultFormat.DateTimeFormat.Kind = fkDateTime
    DefaultFormat.FloatFormat.FormatStr = '%2.n'
    DefaultFormat.FloatFormat.Kind = fkNumeric
    DefaultFormat.CurrencyFormat.FormatStr = '%2.2m'
    DefaultFormat.CurrencyFormat.Kind = fkNumeric
    DefaultFormat.IntegerFormat.FormatStr = '%2.n'
    DefaultFormat.IntegerFormat.Kind = fkNumeric
    DefaultFormat.PercentFormat.FormatStr = '%2.2n'
    DefaultFormat.PercentFormat.Kind = fkNumeric
    UseMultiLoad = False
    Hierarchies = <>
    Version = '1.10.3'
    Left = 744
    Top = 224
  end
  object fcSlice1: TfcSlice
    DefaultColWidth = 100
    Options = [mdsoNoTotalIfOneValue]
    Cube = fcCube1
    FieldsOrder = fcfloByFieldName
    ShowSplitFieldsInFieldList = fcssf_Default
    Left = 696
    Top = 224
  end
  object Daily_DB: TUniQuery
    DataTypeMap = <>
    Connection = _DataModule.UniConnection_Mars
    SQL.Strings = (
      
        'SELECT -- dbo.Fstatus_Claim_TP(wbs,c.branch,easap) as Status_Che' +
        'ck,'
      '       '
      '      -- c.branch as kode_branch,'
      '       '
      '       a.PERIODE_CYCLE,a.BRANCH,a.KODE_CAB_IAP,'
      '       '
      
        '       b.SUB_BRAND,b.MATERIAL,b.REF_IAP,b.REF_TMP as Ref_TSP,b.P' +
        'ILAR, b.WBS,b.GROUPING_BY_EASAP as Grouping_name,'
      '       '
      
        '       b.JENIS_BY_EASAP as Jenis,b.SALES_QTY_BASED_UOM,b.NOMINAL' +
        '_KLAIM_IAP,b.VALIDASI_TMP_FINAL as VALIDASI_TSP_FINAL,B.TOLAKAN,'
      ''
      
        '       a.NO_TRANSAKSI,tgl_transaksi,b.easap,a.no_sk,b.STOCK_POIN' +
        'T,b.ORDER_DATE,'
      
        '       b.INVOICE_NUMBER,b.CUSTOMER,b.TYPE_OUTLET,b.SALES_QTY_FRA' +
        'CTION,b.VALIDASI_TMP_ON_PROGRESS as VALIDASI_TSP_ON_PROGRESS,'
      ''
      
        '       b.BELUM_VALIDASI,b.TOLAKAN_BLM_VALIDASI,b.TOLAKAN_SDH_VAL' +
        'IDASI,'
      '       '
      
        '       Case when a.status_n='#39'5'#39' then '#39'Status:Noted from HO'#39' else' +
        ' '#39'-'#39' end as Status,'
      '       '
      '       Case when a.status_n='#39'5'#39' then 5 else 0 end as CStatus,'
      '       tahun_cycle'
      '           '
      ''
      
        '  FROM KLAIM_EXPENSE_TP_M a,KLAIM_EXPENSE_TP_D b -- ,GR_LIST_REK' +
        'ANAN c'
      ''
      'where '
      '  tgl_transaksi>=:0'
      '  and tgl_transaksi<=:1'
      ''
      '  and a.NO_TRANSAKSI=b.NO_TRANSAKSI'
      ' -- and a.branch=c.branch')
    Left = 536
    Top = 174
    ParamData = <
      item
        DataType = ftDate
        Name = '0'
        ParamType = ptInput
        Value = 42736d
      end
      item
        DataType = ftDate
        Name = '1'
        ParamType = ptInput
        Value = 42402d
      end>
    object Daily_DBPERIODE_CYCLE: TIntegerField
      FieldName = 'PERIODE_CYCLE'
    end
    object Daily_DBBRANCH: TStringField
      FieldName = 'BRANCH'
      Size = 50
    end
    object Daily_DBKODE_CAB_IAP: TStringField
      FieldName = 'KODE_CAB_IAP'
      Size = 50
    end
    object Daily_DBSUB_BRAND: TStringField
      FieldName = 'SUB_BRAND'
      ReadOnly = True
      Size = 25
    end
    object Daily_DBMATERIAL: TStringField
      FieldName = 'MATERIAL'
      ReadOnly = True
      Size = 50
    end
    object Daily_DBREF_IAP: TStringField
      FieldName = 'REF_IAP'
      ReadOnly = True
      Size = 50
    end
    object Daily_DBREF_TSP: TStringField
      FieldName = 'REF_TSP'
      ReadOnly = True
      Size = 50
    end
    object Daily_DBPILAR: TStringField
      FieldName = 'PILAR'
      ReadOnly = True
      Size = 50
    end
    object Daily_DBWBS: TStringField
      FieldName = 'WBS'
      ReadOnly = True
      Size = 150
    end
    object Daily_DBGrouping_name: TStringField
      FieldName = 'Grouping_name'
      ReadOnly = True
      Size = 50
    end
    object Daily_DBJENIS: TStringField
      FieldName = 'JENIS'
      ReadOnly = True
      Size = 50
    end
    object Daily_DBSALES_QTY_BASED_UOM: TIntegerField
      FieldName = 'SALES_QTY_BASED_UOM'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBNOMINAL_KLAIM_IAP: TFloatField
      FieldName = 'NOMINAL_KLAIM_IAP'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBVALIDASI_TSP_FINAL: TFloatField
      FieldName = 'VALIDASI_TSP_FINAL'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBTOLAKAN: TFloatField
      FieldName = 'TOLAKAN'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBNO_TRANSAKSI: TStringField
      FieldName = 'NO_TRANSAKSI'
      Required = True
      Size = 50
    end
    object Daily_DBTGL_TRANSAKSI: TDateField
      FieldName = 'TGL_TRANSAKSI'
    end
    object Daily_DBEASAP: TStringField
      FieldName = 'EASAP'
      ReadOnly = True
      Size = 50
    end
    object Daily_DBNO_SK: TStringField
      FieldName = 'NO_SK'
      Size = 255
    end
    object Daily_DBSTOCK_POINT: TStringField
      FieldName = 'STOCK_POINT'
      ReadOnly = True
      Size = 50
    end
    object Daily_DBORDER_DATE: TDateField
      FieldName = 'ORDER_DATE'
      ReadOnly = True
    end
    object Daily_DBINVOICE_NUMBER: TStringField
      FieldName = 'INVOICE_NUMBER'
      ReadOnly = True
      Size = 25
    end
    object Daily_DBCUSTOMER: TStringField
      FieldName = 'CUSTOMER'
      ReadOnly = True
      Size = 255
    end
    object Daily_DBTYPE_OUTLET: TStringField
      FieldName = 'TYPE_OUTLET'
      ReadOnly = True
      Size = 50
    end
    object Daily_DBSALES_QTY_FRACTION: TIntegerField
      FieldName = 'SALES_QTY_FRACTION'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBVALIDASI_TSP_ON_PROGRESS: TFloatField
      FieldName = 'VALIDASI_TSP_ON_PROGRESS'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBBELUM_VALIDASI: TFloatField
      FieldName = 'BELUM_VALIDASI'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBTOLAKAN_BLM_VALIDASI: TFloatField
      FieldName = 'TOLAKAN_BLM_VALIDASI'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBTOLAKAN_SDH_VALIDASI: TFloatField
      FieldName = 'TOLAKAN_SDH_VALIDASI'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBLcabang_IAP: TStringField
      FieldKind = fkLookup
      FieldName = 'Lcabang_IAP'
      LookupDataSet = Mlist_Branch
      LookupKeyFields = 'KODE_CAB_IAP'
      LookupResultField = 'CABANG_IAP'
      KeyFields = 'KODE_CAB_IAP'
      Size = 150
      Lookup = True
    end
    object Daily_DBStatus: TStringField
      FieldName = 'Status'
      ReadOnly = True
      Required = True
    end
    object Daily_DBtahun_cycle: TIntegerField
      FieldName = 'tahun_cycle'
    end
    object Daily_DBCStatus: TIntegerField
      FieldName = 'CStatus'
      ReadOnly = True
      Required = True
    end
  end
  object UniDataSource1: TUniDataSource
    DataSet = Daily_DB
    Left = 460
    Top = 178
  end
  object PopupMenu2: TPopupMenu
    Left = 600
    Top = 241
    object ImportRaw1: TMenuItem
      Caption = 'Eksport - Fast Report'
    end
    object SinkronProduct1: TMenuItem
      Caption = 'Save Template'
    end
  end
  object PopupMenu1: TPopupMenu
    AutoHotkeys = maManual
    Left = 336
    Top = 241
    object MenuItem1: TMenuItem
      Caption = 'Pivot'
    end
    object MenuItem2: TMenuItem
      Caption = 'Fast Report'
    end
  end
  object QExport4Dialog1: TQExport4Dialog
    DataSet = Daily_DB
    OnlyVisibleFields = True
    RTFOptions.CaptionStyle.Font.Charset = DEFAULT_CHARSET
    RTFOptions.CaptionStyle.Font.Color = clBlack
    RTFOptions.CaptionStyle.Font.Height = -13
    RTFOptions.CaptionStyle.Font.Name = 'Arial'
    RTFOptions.CaptionStyle.Font.Style = [fsBold]
    RTFOptions.CaptionStyle.AllowHighlight = True
    RTFOptions.CaptionStyle.Alignment = talCenter
    RTFOptions.DataStyle.Font.Charset = DEFAULT_CHARSET
    RTFOptions.DataStyle.Font.Color = clBlack
    RTFOptions.DataStyle.Font.Height = -13
    RTFOptions.DataStyle.Font.Name = 'Arial'
    RTFOptions.DataStyle.Font.Style = []
    RTFOptions.DataStyle.AllowHighlight = True
    RTFOptions.FooterStyle.Font.Charset = DEFAULT_CHARSET
    RTFOptions.FooterStyle.Font.Color = clBlack
    RTFOptions.FooterStyle.Font.Height = -13
    RTFOptions.FooterStyle.Font.Name = 'Arial'
    RTFOptions.FooterStyle.Font.Style = []
    RTFOptions.FooterStyle.AllowHighlight = True
    RTFOptions.HeaderStyle.Font.Charset = DEFAULT_CHARSET
    RTFOptions.HeaderStyle.Font.Color = clBlack
    RTFOptions.HeaderStyle.Font.Height = -13
    RTFOptions.HeaderStyle.Font.Name = 'Arial'
    RTFOptions.HeaderStyle.Font.Style = []
    RTFOptions.HeaderStyle.AllowHighlight = True
    RTFOptions.StripStyles = <>
    HTMLPageOptions.TextFont.Charset = DEFAULT_CHARSET
    HTMLPageOptions.TextFont.Color = clWhite
    HTMLPageOptions.TextFont.Height = -11
    HTMLPageOptions.TextFont.Name = 'Arial'
    HTMLPageOptions.TextFont.Style = []
    CSVOptions.Comma = ','
    PDFOptions.PageOptions.MarginLeft = 1.170000000000000000
    PDFOptions.PageOptions.MarginRight = 0.570000000000000000
    PDFOptions.PageOptions.MarginTop = 0.780000000000000000
    PDFOptions.PageOptions.MarginBottom = 0.780000000000000000
    PDFOptions.HeaderFont.UserFont.Charset = DEFAULT_CHARSET
    PDFOptions.HeaderFont.UserFont.Color = clWindowText
    PDFOptions.HeaderFont.UserFont.Height = -13
    PDFOptions.HeaderFont.UserFont.Name = 'Arial'
    PDFOptions.HeaderFont.UserFont.Style = []
    PDFOptions.CaptionFont.UserFont.Charset = DEFAULT_CHARSET
    PDFOptions.CaptionFont.UserFont.Color = clWindowText
    PDFOptions.CaptionFont.UserFont.Height = -13
    PDFOptions.CaptionFont.UserFont.Name = 'Arial'
    PDFOptions.CaptionFont.UserFont.Style = []
    PDFOptions.DataFont.UserFont.Charset = DEFAULT_CHARSET
    PDFOptions.DataFont.UserFont.Color = clWindowText
    PDFOptions.DataFont.UserFont.Height = -13
    PDFOptions.DataFont.UserFont.Name = 'Arial'
    PDFOptions.DataFont.UserFont.Style = []
    PDFOptions.FooterFont.UserFont.Charset = DEFAULT_CHARSET
    PDFOptions.FooterFont.UserFont.Color = clWindowText
    PDFOptions.FooterFont.UserFont.Height = -13
    PDFOptions.FooterFont.UserFont.Name = 'Arial'
    PDFOptions.FooterFont.UserFont.Style = []
    XLSOptions.PageFooter = 'Page &P of &N'
    XLSOptions.SheetTitle = 'Sheet 1'
    XLSOptions.CaptionFormat.Font.Style = [xfsBold]
    XLSOptions.HyperlinkFormat.Font.Color = clrBlue
    XLSOptions.HyperlinkFormat.Font.Underline = fulSingle
    XLSOptions.NoteFormat.Alignment.Horizontal = halLeft
    XLSOptions.NoteFormat.Alignment.Vertical = valTop
    XLSOptions.NoteFormat.Font.Size = 8
    XLSOptions.NoteFormat.Font.Style = [xfsBold]
    XLSOptions.NoteFormat.Font.Name = 'Tahoma'
    XLSOptions.FieldFormats = <>
    XLSOptions.StripStyles = <>
    XLSOptions.Hyperlinks = <>
    XLSOptions.Notes = <>
    XLSOptions.Charts = <>
    XLSOptions.Pictures = <>
    XLSOptions.Images = <>
    XLSOptions.Cells = <>
    XLSOptions.MergedCells = <>
    ODSOptions.HeaderStyle.Font.Charset = DEFAULT_CHARSET
    ODSOptions.HeaderStyle.Font.Color = clBlack
    ODSOptions.HeaderStyle.Font.Height = -13
    ODSOptions.HeaderStyle.Font.Name = 'Arial'
    ODSOptions.HeaderStyle.Font.Style = []
    ODSOptions.FooterStyle.Font.Charset = DEFAULT_CHARSET
    ODSOptions.FooterStyle.Font.Color = clBlack
    ODSOptions.FooterStyle.Font.Height = -13
    ODSOptions.FooterStyle.Font.Name = 'Arial'
    ODSOptions.FooterStyle.Font.Style = []
    ODSOptions.CaptionRowStyle.Font.Charset = DEFAULT_CHARSET
    ODSOptions.CaptionRowStyle.Font.Color = clBlack
    ODSOptions.CaptionRowStyle.Font.Height = -13
    ODSOptions.CaptionRowStyle.Font.Name = 'Arial'
    ODSOptions.CaptionRowStyle.Font.Style = []
    ODSOptions.DataStyle.Font.Charset = DEFAULT_CHARSET
    ODSOptions.DataStyle.Font.Color = clBlack
    ODSOptions.DataStyle.Font.Height = -13
    ODSOptions.DataStyle.Font.Name = 'Arial'
    ODSOptions.DataStyle.Font.Style = []
    ODSOptions.StripStylesList = <>
    ODTOptions.HeaderStyle.Font.Charset = DEFAULT_CHARSET
    ODTOptions.HeaderStyle.Font.Color = clBlack
    ODTOptions.HeaderStyle.Font.Height = -13
    ODTOptions.HeaderStyle.Font.Name = 'Arial'
    ODTOptions.HeaderStyle.Font.Style = []
    ODTOptions.FooterStyle.Font.Charset = DEFAULT_CHARSET
    ODTOptions.FooterStyle.Font.Color = clBlack
    ODTOptions.FooterStyle.Font.Height = -13
    ODTOptions.FooterStyle.Font.Name = 'Arial'
    ODTOptions.FooterStyle.Font.Style = []
    ODTOptions.CaptionRowStyle.Font.Charset = DEFAULT_CHARSET
    ODTOptions.CaptionRowStyle.Font.Color = clBlack
    ODTOptions.CaptionRowStyle.Font.Height = -13
    ODTOptions.CaptionRowStyle.Font.Name = 'Arial'
    ODTOptions.CaptionRowStyle.Font.Style = []
    ODTOptions.DataStyle.Font.Charset = DEFAULT_CHARSET
    ODTOptions.DataStyle.Font.Color = clBlack
    ODTOptions.DataStyle.Font.Height = -13
    ODTOptions.DataStyle.Font.Name = 'Arial'
    ODTOptions.DataStyle.Font.Style = []
    ODTOptions.StripStylesList = <>
    ODTOptions.Border.BorderStyle = bsODFSolid
    XlsxOptions.HeaderStyle.Font.Charset = DEFAULT_CHARSET
    XlsxOptions.HeaderStyle.Font.Color = clBlack
    XlsxOptions.HeaderStyle.Font.Height = -15
    XlsxOptions.HeaderStyle.Font.Name = 'Calibri'
    XlsxOptions.HeaderStyle.Font.Style = []
    XlsxOptions.CaptionRowStyle.Font.Charset = DEFAULT_CHARSET
    XlsxOptions.CaptionRowStyle.Font.Color = clBlack
    XlsxOptions.CaptionRowStyle.Font.Height = -15
    XlsxOptions.CaptionRowStyle.Font.Name = 'Calibri'
    XlsxOptions.CaptionRowStyle.Font.Style = []
    XlsxOptions.DataStyle.Font.Charset = DEFAULT_CHARSET
    XlsxOptions.DataStyle.Font.Color = clBlack
    XlsxOptions.DataStyle.Font.Height = -15
    XlsxOptions.DataStyle.Font.Name = 'Calibri'
    XlsxOptions.DataStyle.Font.Style = []
    XlsxOptions.FooterStyle.Font.Charset = DEFAULT_CHARSET
    XlsxOptions.FooterStyle.Font.Color = clBlack
    XlsxOptions.FooterStyle.Font.Height = -15
    XlsxOptions.FooterStyle.Font.Name = 'Calibri'
    XlsxOptions.FooterStyle.Font.Style = []
    XlsxOptions.StripStylesList = <>
    DocxOptions.HeaderStyle.Font.Charset = DEFAULT_CHARSET
    DocxOptions.HeaderStyle.Font.Color = clBlack
    DocxOptions.HeaderStyle.Font.Height = -15
    DocxOptions.HeaderStyle.Font.Name = 'Calibri'
    DocxOptions.HeaderStyle.Font.Style = []
    DocxOptions.CaptionRowStyle.Font.Charset = DEFAULT_CHARSET
    DocxOptions.CaptionRowStyle.Font.Color = clBlack
    DocxOptions.CaptionRowStyle.Font.Height = -15
    DocxOptions.CaptionRowStyle.Font.Name = 'Calibri'
    DocxOptions.CaptionRowStyle.Font.Style = []
    DocxOptions.DataStyle.Font.Charset = DEFAULT_CHARSET
    DocxOptions.DataStyle.Font.Color = clBlack
    DocxOptions.DataStyle.Font.Height = -15
    DocxOptions.DataStyle.Font.Name = 'Calibri'
    DocxOptions.DataStyle.Font.Style = []
    DocxOptions.StripStylesList = <>
    DocxOptions.FooterStyle.Font.Charset = DEFAULT_CHARSET
    DocxOptions.FooterStyle.Font.Color = clBlack
    DocxOptions.FooterStyle.Font.Height = -15
    DocxOptions.FooterStyle.Font.Name = 'Calibri'
    DocxOptions.FooterStyle.Font.Style = []
    AccessOptions.TableName = 'EXPORT_TABLE'
    Left = 924
    Top = 205
  end
  object PopupMenu3: TPopupMenu
    Left = 276
    Top = 245
    object Grid1: TMenuItem
      Caption = 'Grid'
      OnClick = Grid1Click
    end
    object DataRaw1: TMenuItem
      Caption = 'Data Raw'
      OnClick = DataRaw1Click
    end
  end
  object Mlist_Branch: TUniQuery
    KeyFields = 'KODE_BRANCH;KODE_CAB_IAP'
    DataTypeMap = <>
    Connection = _DataModule.UniConnection_Mars
    SQL.Strings = (
      'SELECT a.*'
      'FROM dbo.LIST_BRANCH_IAP a')
    MasterFields = 'Lkode_Branch'
    DetailFields = 'KODE_BRANCH'
    Active = True
    Left = 496
    Top = 321
    object Mlist_BranchCABANG_IAP: TStringField
      DisplayWidth = 150
      FieldName = 'CABANG_IAP'
      Required = True
      Size = 150
    end
    object Mlist_BranchKODE_CAB_IAP: TStringField
      DisplayWidth = 50
      FieldName = 'KODE_CAB_IAP'
      Required = True
      Size = 50
    end
    object Mlist_BranchBRANCH: TStringField
      DisplayWidth = 50
      FieldName = 'BRANCH'
      Required = True
      Visible = False
      Size = 50
    end
    object Mlist_BranchKODE_BRANCH: TStringField
      DisplayWidth = 5
      FieldName = 'KODE_BRANCH'
      Required = True
      Visible = False
      Size = 5
    end
  end
  object Master: TUniQuery
    DataTypeMap = <>
    Connection = _DataModule.UniConnection_Mars
    SQL.Strings = (
      'select * from KLAIM_EXPENSE_TP_M a')
    Left = 156
    Top = 168
    object MasterID_TRANSAKSI: TIntegerField
      FieldName = 'ID_TRANSAKSI'
      Required = True
    end
    object MasterKODE: TStringField
      FieldName = 'KODE'
    end
    object MasterNO_TRANSAKSI: TStringField
      FieldName = 'NO_TRANSAKSI'
      Required = True
      Size = 50
    end
    object MasterKETERANGAN: TStringField
      FieldName = 'KETERANGAN'
      Size = 255
    end
    object MasterNAMA_TTD1: TStringField
      FieldName = 'NAMA_TTD1'
      Size = 255
    end
    object MasterNAMA_TTD2: TStringField
      FieldName = 'NAMA_TTD2'
      Size = 255
    end
    object MasterNAMA_TTD3: TStringField
      FieldName = 'NAMA_TTD3'
      Size = 255
    end
    object MasterNAMA_TTD4: TStringField
      FieldName = 'NAMA_TTD4'
      Size = 255
    end
    object MasterTGL_TTD1: TDateTimeField
      FieldName = 'TGL_TTD1'
    end
    object MasterTGL_TTD2: TDateTimeField
      FieldName = 'TGL_TTD2'
    end
    object MasterTGL_TTD3: TDateTimeField
      FieldName = 'TGL_TTD3'
    end
    object MasterTGL_TTD4: TDateTimeField
      FieldName = 'TGL_TTD4'
    end
    object MasterTGL_TRANSAKSI: TDateField
      FieldName = 'TGL_TRANSAKSI'
    end
    object MasterOPR_INSERT: TStringField
      FieldName = 'OPR_INSERT'
      Size = 100
    end
    object MasterTGL_INSERT: TDateTimeField
      FieldName = 'TGL_INSERT'
    end
    object MasterOPR_EDIT: TStringField
      FieldName = 'OPR_EDIT'
      Size = 100
    end
    object MasterTGL_EDIT: TDateTimeField
      FieldName = 'TGL_EDIT'
    end
    object MasterOPR_TERMINATED: TStringField
      FieldName = 'OPR_TERMINATED'
      Size = 100
    end
    object MasterTGL_TERMINATED: TDateTimeField
      FieldName = 'TGL_TERMINATED'
    end
    object MasterREASON_TERMINATED: TStringField
      FieldName = 'REASON_TERMINATED'
      Size = 100
    end
    object MasterSTATUS: TStringField
      FieldName = 'STATUS'
      Size = 1
    end
    object MasterOLD_STATUS: TStringField
      FieldName = 'OLD_STATUS'
      Size = 1
    end
    object MasterEXPLANATION: TStringField
      FieldName = 'EXPLANATION'
      Size = 250
    end
    object MasterBRANCHLAMA: TStringField
      FieldName = 'BRANCHLAMA'
      Size = 50
    end
    object MasterCOMPANY: TStringField
      FieldName = 'COMPANY'
      Size = 150
    end
    object MasterDIVISI: TStringField
      FieldName = 'DIVISI'
      Size = 150
    end
    object MasterBULAN: TIntegerField
      FieldName = 'BULAN'
    end
    object MasterTAHUN: TIntegerField
      FieldName = 'TAHUN'
    end
    object MasterBRANCH: TStringField
      FieldName = 'BRANCH'
      Size = 50
    end
    object MasterPERIODE: TIntegerField
      FieldName = 'PERIODE'
    end
    object MasterKODE_CABANG: TStringField
      FieldName = 'KODE_CABANG'
      Size = 10
    end
    object MasterCABANG: TStringField
      FieldName = 'CABANG'
      Size = 255
    end
    object MasterTGL_DOKUMEN: TDateField
      FieldName = 'TGL_DOKUMEN'
    end
    object MasterTGL_DOK_DITERIMA: TDateField
      FieldName = 'TGL_DOK_DITERIMA'
    end
    object MasterTGL_VALIDASI: TDateField
      FieldName = 'TGL_VALIDASI'
    end
    object MasterNO_SK: TStringField
      FieldName = 'NO_SK'
      Size = 255
    end
    object MasterNO_SKP: TStringField
      FieldName = 'NO_SKP'
      Size = 255
    end
    object MasterBRANCH_LAMA: TStringField
      FieldName = 'BRANCH_LAMA'
      Size = 50
    end
    object MasterKODE_CAB_IAP: TStringField
      FieldName = 'KODE_CAB_IAP'
      Size = 50
    end
    object MasterPERIODE_CYCLE: TIntegerField
      FieldName = 'PERIODE_CYCLE'
    end
    object Masterstatus_n: TStringField
      FieldName = 'status_n'
      Size = 1
    end
    object MasterTahun_cycle: TIntegerField
      FieldName = 'Tahun_cycle'
    end
  end
  object Detil: TUniQuery
    DataTypeMap = <>
    Connection = _DataModule.UniConnection_Mars
    SQL.Strings = (
      'select * from KLAIM_EXPENSE_TP_D a')
    Left = 212
    Top = 168
    object DetilNO_TRANSAKSI: TStringField
      FieldName = 'NO_TRANSAKSI'
      Required = True
      Size = 50
    end
    object DetilSUB_BRAND: TStringField
      FieldName = 'SUB_BRAND'
      Size = 25
    end
    object DetilMATERIAL: TStringField
      FieldName = 'MATERIAL'
      Size = 50
    end
    object DetilSTOCK_POINT: TStringField
      FieldName = 'STOCK_POINT'
      Size = 50
    end
    object DetilORDER_DATE: TDateField
      FieldName = 'ORDER_DATE'
    end
    object DetilINVOICE_NUMBER: TStringField
      FieldName = 'INVOICE_NUMBER'
      Size = 25
    end
    object DetilCUSTOMER: TStringField
      FieldName = 'CUSTOMER'
      Size = 255
    end
    object DetilTYPE_OUTLET: TStringField
      FieldName = 'TYPE_OUTLET'
      Size = 50
    end
    object DetilSALES_QTY_BASED_UOM: TIntegerField
      FieldName = 'SALES_QTY_BASED_UOM'
    end
    object DetilSALES_QTY_FRACTION: TIntegerField
      FieldName = 'SALES_QTY_FRACTION'
    end
    object DetilNOMINAL_KLAIM_IAP: TFloatField
      FieldName = 'NOMINAL_KLAIM_IAP'
    end
    object DetilVALIDASI_TMP_FINAL: TFloatField
      FieldName = 'VALIDASI_TMP_FINAL'
    end
    object DetilVALIDASI_TMP_ON_PROGRESS: TFloatField
      FieldName = 'VALIDASI_TMP_ON_PROGRESS'
    end
    object DetilBELUM_VALIDASI: TFloatField
      FieldName = 'BELUM_VALIDASI'
    end
    object DetilTOLAKAN: TFloatField
      FieldName = 'TOLAKAN'
    end
    object DetilTOLAKAN_BLM_VALIDASI: TFloatField
      FieldName = 'TOLAKAN_BLM_VALIDASI'
    end
    object DetilTOLAKAN_SDH_VALIDASI: TFloatField
      FieldName = 'TOLAKAN_SDH_VALIDASI'
    end
    object DetilREF_IAP: TStringField
      FieldName = 'REF_IAP'
      Size = 50
    end
    object DetilREF_TMP: TStringField
      FieldName = 'REF_TMP'
      Size = 50
    end
    object DetilEASAP: TStringField
      FieldName = 'EASAP'
      Size = 50
    end
    object DetilEASAP_APPRV: TStringField
      FieldName = 'EASAP_APPRV'
      Size = 50
    end
    object DetilGL_EASAP: TStringField
      FieldName = 'GL_EASAP'
      Size = 50
    end
    object DetilPERIODE_DLM_EASAP: TStringField
      FieldName = 'PERIODE_DLM_EASAP'
      Size = 50
    end
    object DetilJENIS_DLM_EASAP: TStringField
      FieldName = 'JENIS_DLM_EASAP'
      Size = 50
    end
    object DetilKETERANGAN: TStringField
      FieldName = 'KETERANGAN'
      Size = 255
    end
    object DetilPILAR: TStringField
      FieldName = 'PILAR'
      Size = 50
    end
    object DetilGROUPING_BY_EASAP: TStringField
      FieldName = 'GROUPING_BY_EASAP'
      Size = 50
    end
    object DetilJENIS_BY_EASAP: TStringField
      FieldName = 'JENIS_BY_EASAP'
      Size = 50
    end
    object DetilGROUPING_BY_CABANG: TStringField
      FieldName = 'GROUPING_BY_CABANG'
      Size = 50
    end
    object DetilJENIS_BY_CABANG: TStringField
      FieldName = 'JENIS_BY_CABANG'
      Size = 50
    end
    object DetilWBS: TStringField
      FieldName = 'WBS'
      Size = 150
    end
  end
  object QImport3Wizard5: TQImport3Wizard
    Formats.LongDateFormat = 'dd/MM/yyyy'
    Formats.BooleanTrue.Strings = (
      'True')
    Formats.BooleanFalse.Strings = (
      'False')
    Formats.NullValues.Strings = (
      'Null')
    FieldFormats = <>
    ErrorLogFileName = 'error.log'
    AddType = qatInsert
    Left = 160
    Top = 223
  end
end
