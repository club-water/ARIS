...
type
  TInf_Kas_Harian_DetilFrm = class(TForm)
    Panel1: TPanel;
    Label3: TLabel;
    ...
    PerOrang1: TMenuItem;
    Daily_DBSUBUNIT_ID: TIntegerField;
    Daily_DBTglDoc: TDateTimeField;
    Daily_DBKETBIPDPT: TStringField;
    Daily_DBNILTUNAI: TFloatField;
    Daily_DBNODOCU: TStringField;
    Daily_DBKETERANGAN: TStringField;
    Daily_DBKDDOCU: TStringField;
    Daily_DBKDACC: TStringField;
    Daily_DBKETACCOUNT: TStringField;
    cxGrid3DBTableView1TglDoc: TcxGridDBColumn;
    cxGrid3DBTableView1KETBIPDPT: TcxGridDBColumn;
    cxGrid3DBTableView1NILTUNAI: TcxGridDBColumn;
    ...
    cxGrid3DBTableView1KETACCOUNT: TcxGridDBColumn;
...
var
  Inf_Kas_Harian_DetilFrm: TInf_Kas_Harian_DetilFrm;
implementation
...
procedure TInf_Kas_Harian_DetilFrm.MenuClick(Sender: TObject);
...
begin
      ...
      FileName:=ExtractFileDir(Application.ExeName)+'\KASDETIL\'+my_text;
...
procedure TInf_Kas_Harian_DetilFrm.Set_Pop_up_monClick(Sender: TObject);
var FileName,Path :String;
    i :integer;
    NewItem : TMenuItem;
    Vnama :String;
begin
  FileName:=ExtractFileDir(Application.ExeName)+'\KASDETIL';
  try
  MkDir(FileName);
  except
  end;
  ...
  DirectoryListBox1.Directory:=ExtractFileDir(Application.ExeName)+'\KASDETIL';
...
{ Clear PopupItems }
  ...
  PATH:=ExtractFileDir(Application.ExeName)+'\KASDETIL';
...
procedure TInf_Kas_Harian_DetilFrm.FormClose(Sender: TObject; var Action: TCloseAction);
var FileName :String;
begin
 FileName:=ExtractFileDir(Application.ExeName)+'\kas_harian_Detil.mds';
 if (not fcCube1.DataSet.IsEmpty) and (fcSlice1.HaveLayout) then
  ...
  Action := caFree;
  Inf_Kas_Harian_DetilFrm := nil;
end;

procedure TInf_Kas_Harian_DetilFrm.BitBtn1Click(Sender: TObject);
var AIndex: integer;
    Vmenit,Vdetik : integer;
    FileName :String;
begin
...
procedure TInf_Kas_Harian_DetilFrm.BitBtn2Click(Sender: TObject);
begin
  close;
end;

procedure TInf_Kas_Harian_DetilFrm.FormShow(Sender: TObject);
begin
...
procedure TInf_Kas_Harian_DetilFrm.Button7Click(Sender: TObject);
var i:integer;
    vlist:string;
begin
  fcCube1.Active := false;
...
procedure TInf_Kas_Harian_DetilFrm.LMDButton15Click(Sender: TObject);
begin
...
procedure TInf_Kas_Harian_DetilFrm.Button1Click(Sender: TObject);
begin
  cxGrid3DBTableView1.ApplyBestFit;
end;

procedure TInf_Kas_Harian_DetilFrm.Button2Click(Sender: TObject);
var FileName :String;
begin
try
 FileName:=ExtractFileDir(Application.ExeName)+'\kas_harian_Detil Grids.ini';
...
procedure TInf_Kas_Harian_DetilFrm.Button3Click(Sender: TObject);
var FileName :String;
begin
  FileName:=ExtractFileDir(Application.ExeName)+'\KASDETIL';
