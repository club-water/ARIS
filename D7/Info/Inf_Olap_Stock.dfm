object Inf_Olap_StockFrm: TInf_Olap_StockFrm
  Left = 105
  Top = 105
  Width = 1223
  Height = 608
  Caption = 'Inf_Olap_StockFrm'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1207
    Height = 45
    Align = alTop
    TabOrder = 0
    object Label3: TLabel
      Left = 4
      Top = 2
      Width = 48
      Height = 13
      Caption = 'Periode:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object Label4: TLabel
      Left = 833
      Top = 24
      Width = 32
      Height = 13
      Caption = 'Label4'
      Visible = False
    end
    object Label5: TLabel
      Left = 752
      Top = 24
      Width = 76
      Height = 13
      Caption = 'Loading Time  : '
      Visible = False
    end
    object Label6: TLabel
      Left = 895
      Top = 5
      Width = 32
      Height = 13
      Caption = 'Label6'
      Visible = False
    end
    object Label7: TLabel
      Left = 752
      Top = 6
      Width = 32
      Height = 13
      Caption = 'Label7'
      Visible = False
    end
    object Label2: TLabel
      Left = 107
      Top = 19
      Width = 31
      Height = 20
      Caption = 'S.D'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object ToDate: TDateTimePicker
      Left = 143
      Top = 18
      Width = 101
      Height = 21
      Date = 40772.000000000000000000
      Time = 40772.000000000000000000
      TabOrder = 3
    end
    object FromDate: TDateTimePicker
      Left = 7
      Top = 18
      Width = 97
      Height = 21
      Date = 40909.000000000000000000
      Time = 40909.000000000000000000
      TabOrder = 0
    end
    object BitBtn1: TBitBtn
      Left = 255
      Top = 5
      Width = 41
      Height = 38
      Hint = 'Refresh'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        1F7C1F7C1F7C}
    end
    object BitBtn2: TBitBtn
      Left = 301
      Top = 5
      Width = 41
      Height = 38
      Hint = 'Close'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        1F7C1F7C1F7C}
    end
    object LMDButton1: TLMDButton
      Left = 1004
      Top = 2
      Width = 128
      Height = 29
      Caption = 'Generated Report'
      PopupMenu = PopupMenu1
      TabOrder = 4
      Visible = False
      ButtonLayout.Arrow = True
      DropDownMenu = PopupMenu1
    end
  end
  object Filter: TPageControl
    Left = 0
    Top = 45
    Width = 1207
    Height = 524
    ActivePage = TabSheet3
    Align = alClient
    TabOrder = 1
    object TabSheet1: TTabSheet
      Caption = 'Rekapitulasi'
      object Panel2: TPanel
        Left = 0
        Top = 0
        Width = 1199
        Height = 41
        Align = alTop
        TabOrder = 0
        object LMDButton2: TLMDButton
          Left = 6
          Top = 5
          Width = 117
          Height = 30
          Caption = 'Export2Excell'
          TabOrder = 0
          OnClick = LMDButton2Click
          Glyph.Data = {
            
            1F7C1F7C1F7C}
        end
        object Button2: TButton
          Left = 136
          Top = 5
          Width = 117
          Height = 30
          Caption = 'Auto Width'
          TabOrder = 1
          OnClick = Button2Click
        end
      end
      object cxGrid1: TcxGrid
        Left = 0
        Top = 41
        Width = 1199
        Height = 455
        Align = alClient
        TabOrder = 1
        object cxGridDBTableView1: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          DataController.DataSource = UniDataSource1
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'QTY'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'QTYTUNAI'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'QTYKREDIT'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'GROSS'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'GROSSKREDIT'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'GROSSTUNAI'
            end
            item
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'NETTO'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'NETTOKREDIT'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'NETTOTUNAI'
            end>
          DataController.Summary.SummaryGroups = <>
          OptionsView.ColumnAutoWidth = True
          OptionsView.Footer = True
          Styles.ContentEven = _DataModule.cxStyle1
          Styles.ContentOdd = _DataModule.cxStyle3
          object cxGridDBTableView1SUBUNIT: TcxGridDBColumn
            DataBinding.FieldName = 'SUBUNIT'
            Width = 74
          end
          object cxGridDBTableView1KETSUBUNIT: TcxGridDBColumn
            DataBinding.FieldName = 'KETSUBUNIT'
            Width = 158
          end
          object cxGridDBTableView1GUDANG: TcxGridDBColumn
            DataBinding.FieldName = 'GUDANG'
            Width = 110
          end
          object cxGridDBTableView1KETGUDANG: TcxGridDBColumn
            DataBinding.FieldName = 'KETGUDANG'
            Width = 120
          end
          object cxGridDBTableView1BARANG: TcxGridDBColumn
            DataBinding.FieldName = 'BARANG'
            Width = 117
          end
          object cxGridDBTableView1KETBARANG: TcxGridDBColumn
            DataBinding.FieldName = 'KETBARANG'
            Width = 270
          end
          object cxGridDBTableView1saldo_awal: TcxGridDBColumn
            Caption = 'SALDO AWAL'
            DataBinding.FieldName = 'saldo_awal'
            Width = 106
          end
          object cxGridDBTableView1QTYIN: TcxGridDBColumn
            Caption = 'QTY IN'
            DataBinding.FieldName = 'QTYIN'
            Width = 81
          end
          object cxGridDBTableView1QTYOUT: TcxGridDBColumn
            Caption = 'QTY OUT'
            DataBinding.FieldName = 'QTYOUT'
            Width = 80
          end
          object cxGridDBTableView1Saldo_akhir: TcxGridDBColumn
            Caption = 'SALDO AKHIR'
            DataBinding.FieldName = 'Saldo_akhir'
            Width = 81
          end
        end
        object cxGridLevel1: TcxGridLevel
          GridView = cxGridDBTableView1
        end
      end
    end
    object TabSheet2: TTabSheet
      Caption = 'Detil'
      ImageIndex = 2
      object cxGrid3: TcxGrid
        Left = 0
        Top = 41
        Width = 1199
        Height = 455
        Align = alClient
        TabOrder = 0
        object cxGrid3DBTableView1: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          DataController.DataSource = UniDataSource2
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'QTYIN'
              Column = cxGrid3DBTableView1QTYIN
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'QTYOUT'
              Column = cxGrid3DBTableView1QTYOUT
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'QTYKREDIT'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'GROSS'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'GROSSKREDIT'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'GROSSTUNAI'
            end
            item
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'NETTO'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'NETTOKREDIT'
            end
            item
              Format = '#,##0.;(#,##0.)'
              Kind = skSum
              FieldName = 'NETTOTUNAI'
            end>
          DataController.Summary.SummaryGroups = <>
          OptionsView.Footer = True
          Styles.ContentEven = _DataModule.cxStyle1
          Styles.ContentOdd = _DataModule.cxStyle3
          object cxGrid3DBTableView1TGLDOCU: TcxGridDBColumn
            DataBinding.FieldName = 'TGLDOCU'
          end
          object cxGrid3DBTableView1SUBUNIT: TcxGridDBColumn
            DataBinding.FieldName = 'SUBUNIT'
            Width = 75
          end
          object cxGrid3DBTableView1KETSUBUNIT: TcxGridDBColumn
            DataBinding.FieldName = 'KETSUBUNIT'
            Width = 127
          end
          object cxGrid3DBTableView1GUDANG: TcxGridDBColumn
            DataBinding.FieldName = 'GUDANG'
            Width = 71
          end
          object cxGrid3DBTableView1KETGUDANG: TcxGridDBColumn
            DataBinding.FieldName = 'KETGUDANG'
            Width = 215
          end
          object cxGrid3DBTableView1BARANG: TcxGridDBColumn
            DataBinding.FieldName = 'BARANG'
            Width = 82
          end
          object cxGrid3DBTableView1KETBARANG: TcxGridDBColumn
            DataBinding.FieldName = 'KETBARANG'
            Width = 135
          end
          object cxGrid3DBTableView1MODUL: TcxGridDBColumn
            DataBinding.FieldName = 'MODUL'
            Width = 51
          end
          object cxGrid3DBTableView1KDREGS: TcxGridDBColumn
            DataBinding.FieldName = 'KDREGS'
            Width = 73
          end
          object cxGrid3DBTableView1KDDOCU: TcxGridDBColumn
            DataBinding.FieldName = 'KDDOCU'
            Width = 91
          end
          object cxGrid3DBTableView1NODOCU: TcxGridDBColumn
            DataBinding.FieldName = 'NODOCU'
            Width = 91
          end
          object cxGrid3DBTableView1REFERENSI: TcxGridDBColumn
            DataBinding.FieldName = 'REFERENSI'
            Width = 113
          end
          object cxGrid3DBTableView1KETERANGAN: TcxGridDBColumn
            DataBinding.FieldName = 'KETERANGAN'
            Width = 360
          end
          object cxGrid3DBTableView1QTYIN: TcxGridDBColumn
            DataBinding.FieldName = 'QTYIN'
            Width = 101
          end
          object cxGrid3DBTableView1QTYOUT: TcxGridDBColumn
            DataBinding.FieldName = 'QTYOUT'
            Width = 83
          end
        end
        object cxGrid3Level1: TcxGridLevel
          GridView = cxGrid3DBTableView1
        end
      end
      object Panel3: TPanel
        Left = 0
        Top = 0
        Width = 1199
        Height = 41
        Align = alTop
        TabOrder = 1
        object LMDButton15: TLMDButton
          Left = 6
          Top = 5
          Width = 117
          Height = 30
          Caption = 'Export2Excell'
          TabOrder = 0
          OnClick = LMDButton15Click
          Glyph.Data = {
            
            1F7C1F7C1F7C}
        end
        object Button1: TButton
          Left = 256
          Top = 5
          Width = 117
          Height = 30
          Caption = 'Auto Width'
          TabOrder = 1
          OnClick = Button1Click
        end
        object Button3: TButton
          Left = 128
          Top = 5
          Width = 117
          Height = 30
          Caption = 'Save Template'
          TabOrder = 2
          OnClick = Button3Click
        end
      end
    end
    object TabSheet3: TTabSheet
      Caption = 'Pivot'
      ImageIndex = 2
      object Panel4: TPanel
        Left = 0
        Top = 0
        Width = 1199
        Height = 24
        Align = alTop
        TabOrder = 0
        object fcToolBar1: TfcToolBar
          Left = 1
          Top = 1
          Width = 1197
          Height = 22
          Align = alClient
          AutoSize = True
          Caption = 'fcToolBar1'
          Color = clBtnFace
          EdgeBorders = []
          Flat = True
          ParentColor = False
          TabOrder = 0
          Grid = fcGrid1
        end
      end
      object fcGrid1: TfcGrid
        Left = 0
        Top = 24
        Width = 1199
        Height = 472
        DefaultColWidth = 100
        FieldsImages = FieldsImages
        Styles.CaptionArea.FillColor = 15466495
        Styles.CaptionArea.TextColor = clBlack
        Styles.CaptionArea.Font.Charset = DEFAULT_CHARSET
        Styles.CaptionArea.Font.Color = clBlack
        Styles.CaptionArea.Font.Height = -11
        Styles.CaptionArea.Font.Name = 'MS Sans Serif'
        Styles.CaptionArea.Font.Style = []
        Styles.HeaderArea.FillColor = clBtnFace
        Styles.HeaderArea.TextColor = clGray
        Styles.HeaderArea.Font.Charset = DEFAULT_CHARSET
        Styles.HeaderArea.Font.Color = clGray
        Styles.HeaderArea.Font.Height = -11
        Styles.HeaderArea.Font.Name = 'MS Sans Serif'
        Styles.HeaderArea.Font.Style = []
        Styles.DataArea.FillColor = clWhite
        Styles.DataArea.TextColor = clGray
        Styles.DataArea.Font.Charset = DEFAULT_CHARSET
        Styles.DataArea.Font.Color = clGray
        Styles.DataArea.Font.Height = -11
        Styles.DataArea.Font.Name = 'MS Sans Serif'
        Styles.DataArea.Font.Style = []
        Styles.ServiceArea.FillColor = 15466495
        Styles.ServiceArea.TextColor = clBlack
        Styles.ServiceArea.Font.Charset = DEFAULT_CHARSET
        Styles.ServiceArea.Font.Color = clBlack
        Styles.ServiceArea.Font.Height = -11
        Styles.ServiceArea.Font.Name = 'MS Sans Serif'
        Styles.ServiceArea.Font.Style = []
        Styles.ActiveItem.FillColor = clActiveCaption
        Styles.ActiveItem.TextColor = clCaptionText
        Styles.ActiveItem.Font.Charset = DEFAULT_CHARSET
        Styles.ActiveItem.Font.Color = clCaptionText
        Styles.ActiveItem.Font.Height = -11
        Styles.ActiveItem.Font.Name = 'MS Sans Serif'
        Styles.ActiveItem.Font.Style = []
        Styles.InactiveItem.FillColor = clInactiveCaption
        Styles.InactiveItem.TextColor = clInactiveCaptionText
        Styles.InactiveItem.Font.Charset = DEFAULT_CHARSET
        Styles.InactiveItem.Font.Color = clInactiveCaptionText
        Styles.InactiveItem.Font.Height = -11
        Styles.InactiveItem.Font.Name = 'MS Sans Serif'
        Styles.InactiveItem.Font.Style = []
        Styles.MeasureItem.FillColor = clMoneyGreen
        Styles.MeasureItem.TextColor = clCaptionText
        Styles.MeasureItem.Font.Charset = DEFAULT_CHARSET
        Styles.MeasureItem.Font.Color = clCaptionText
        Styles.MeasureItem.Font.Height = -11
        Styles.MeasureItem.Font.Name = 'MS Sans Serif'
        Styles.MeasureItem.Font.Style = []
        Styles.FieldsItem.FillColor = clMoneyGreen
        Styles.FieldsItem.TextColor = clCaptionText
        Styles.FieldsItem.Font.Charset = DEFAULT_CHARSET
        Styles.FieldsItem.Font.Color = clCaptionText
        Styles.FieldsItem.Font.Height = -11
        Styles.FieldsItem.Font.Name = 'MS Sans Serif'
        Styles.FieldsItem.Font.Style = []
        Styles.AxisCells.FillColor = clBtnFace
        Styles.AxisCells.TextColor = clBlack
        Styles.AxisCells.Font.Charset = DEFAULT_CHARSET
        Styles.AxisCells.Font.Color = clBlack
        Styles.AxisCells.Font.Height = -11
        Styles.AxisCells.Font.Name = 'MS Sans Serif'
        Styles.AxisCells.Font.Style = []
        Styles.AxisCellsSelected.FillColor = clBtnShadow
        Styles.AxisCellsSelected.TextColor = clBtnText
        Styles.AxisCellsSelected.Font.Charset = DEFAULT_CHARSET
        Styles.AxisCellsSelected.Font.Color = clBtnText
        Styles.AxisCellsSelected.Font.Height = -11
        Styles.AxisCellsSelected.Font.Name = 'MS Sans Serif'
        Styles.AxisCellsSelected.Font.Style = []
        Styles.DataCells.FillColor = clWhite
        Styles.DataCells.TextColor = clBlack
        Styles.DataCells.Font.Charset = DEFAULT_CHARSET
        Styles.DataCells.Font.Color = clBlack
        Styles.DataCells.Font.Height = -11
        Styles.DataCells.Font.Name = 'MS Sans Serif'
        Styles.DataCells.Font.Style = []
        Styles.DataCellsTotals.FillColor = 2595309
        Styles.DataCellsTotals.TextColor = clBlack
        Styles.DataCellsTotals.Font.Charset = DEFAULT_CHARSET
        Styles.DataCellsTotals.Font.Color = clBlack
        Styles.DataCellsTotals.Font.Height = -11
        Styles.DataCellsTotals.Font.Name = 'MS Sans Serif'
        Styles.DataCellsTotals.Font.Style = []
        Styles.DataCellsSelected.FillColor = clHighlight
        Styles.DataCellsSelected.TextColor = clHighlightText
        Styles.DataCellsSelected.Font.Charset = DEFAULT_CHARSET
        Styles.DataCellsSelected.Font.Color = clHighlightText
        Styles.DataCellsSelected.Font.Height = -11
        Styles.DataCellsSelected.Font.Name = 'MS Sans Serif'
        Styles.DataCellsSelected.Font.Style = []
        Styles.UseParentFont = False
        Slice = fcSlice1
        Align = alClient
        TabOrder = 1
      end
    end
  end
  object FieldsImages: TImageList
    Left = 600
    Top = 292
    Bitmap = {
      
      000000000000}
  end
  object Daily_DB: TUniQuery
    Connection = _DataModule.Report_Con
    SQL.Strings = (
      'EXEC'#9'[dbo].[RPT_KARTUSTOCK] :0,:1,:2,:3,:4')
    Left = 491
    Top = 182
    ParamData = <
      item
        DataType = ftDate
        Name = '0'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = '1'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '2'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '3'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '4'
        ParamType = ptInput
      end>
    object Daily_DBSUBUNIT: TStringField
      FieldName = 'SUBUNIT'
      Required = True
      Size = 4
    end
    object Daily_DBKETSUBUNIT: TStringField
      FieldName = 'KETSUBUNIT'
      Required = True
      Size = 50
    end
    object Daily_DBGUDANG: TStringField
      FieldName = 'GUDANG'
      ReadOnly = True
      Required = True
      Size = 4
    end
    object Daily_DBKETGUDANG: TStringField
      FieldName = 'KETGUDANG'
      ReadOnly = True
      Required = True
      Size = 50
    end
    object Daily_DBBARANG: TStringField
      FieldName = 'BARANG'
      ReadOnly = True
      Required = True
      Size = 10
    end
    object Daily_DBKETBARANG: TStringField
      FieldName = 'KETBARANG'
      ReadOnly = True
      Required = True
      Size = 50
    end
    object Daily_DBsaldo_awal: TIntegerField
      FieldName = 'saldo_awal'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBQTYIN: TIntegerField
      FieldName = 'QTYIN'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBQTYOUT: TIntegerField
      FieldName = 'QTYOUT'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DBSaldo_akhir: TIntegerField
      FieldName = 'Saldo_akhir'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
  end
  object DataSource1: TDataSource
    Left = 716
    Top = 97
  end
  object OpenDialog4: TOpenDialog
    Filter = 'All Report File|*.rtm'
    Left = 176
    Top = 320
  end
  object UniDataSource1: TUniDataSource
    DataSet = Daily_DB
    Left = 420
    Top = 186
  end
  object PopupMenu1: TPopupMenu
    AutoHotkeys = maManual
    Left = 408
    Top = 249
    object MenuItem1: TMenuItem
      Caption = 'Pivot'
    end
    object MenuItem2: TMenuItem
      Caption = 'Fast Report'
    end
  end
  object UniDataSource2: TUniDataSource
    DataSet = Daily_DB_Detil
    Left = 644
    Top = 194
  end
  object Daily_DB_Detil: TUniQuery
    Connection = _DataModule.Report_Con
    SQL.Strings = (
      'EXEC'#9'[dbo].[RPT_KARTUSTOCK_DETIL] :0,:1,:2,:3,:4')
    Active = True
    Left = 715
    Top = 190
    ParamData = <
      item
        DataType = ftDate
        Name = '0'
        ParamType = ptInput
        Value = 41395d
      end
      item
        DataType = ftDate
        Name = '1'
        ParamType = ptInput
        Value = 41421d
      end
      item
        DataType = ftString
        Name = '2'
        ParamType = ptInput
        Value = '1'
      end
      item
        DataType = ftString
        Name = '3'
        ParamType = ptInput
        Value = '1'
      end
      item
        DataType = ftString
        Name = '4'
        ParamType = ptInput
        Value = 'EKO-IT'
      end>
    object Daily_DB_DetilRID: TStringField
      FieldName = 'RID'
      ReadOnly = True
      Required = True
      Size = 1
    end
    object Daily_DB_DetilSUBUNIT: TStringField
      FieldName = 'SUBUNIT'
      ReadOnly = True
      Required = True
      Size = 4
    end
    object Daily_DB_DetilKETSUBUNIT: TStringField
      FieldName = 'KETSUBUNIT'
      ReadOnly = True
      Required = True
      Size = 50
    end
    object Daily_DB_DetilGUDANG: TStringField
      FieldName = 'GUDANG'
      ReadOnly = True
      Required = True
      Size = 4
    end
    object Daily_DB_DetilKETGUDANG: TStringField
      FieldName = 'KETGUDANG'
      ReadOnly = True
      Required = True
      Size = 50
    end
    object Daily_DB_DetilBARANG: TStringField
      FieldName = 'BARANG'
      ReadOnly = True
      Required = True
      Size = 10
    end
    object Daily_DB_DetilKETBARANG: TStringField
      FieldName = 'KETBARANG'
      ReadOnly = True
      Required = True
      Size = 50
    end
    object Daily_DB_DetilTGLDOCU: TDateTimeField
      FieldName = 'TGLDOCU'
      ReadOnly = True
    end
    object Daily_DB_DetilMODUL: TStringField
      FieldName = 'MODUL'
      ReadOnly = True
      Size = 3
    end
    object Daily_DB_DetilKDREGS: TStringField
      FieldName = 'KDREGS'
      ReadOnly = True
      Required = True
      Size = 5
    end
    object Daily_DB_DetilKDDOCU: TStringField
      FieldName = 'KDDOCU'
      ReadOnly = True
      Required = True
      Size = 5
    end
    object Daily_DB_DetilNODOCU: TStringField
      FieldName = 'NODOCU'
      ReadOnly = True
      Required = True
      Size = 6
    end
    object Daily_DB_DetilREFERENSI: TStringField
      FieldName = 'REFERENSI'
      ReadOnly = True
    end
    object Daily_DB_DetilKETERANGAN: TStringField
      FieldName = 'KETERANGAN'
      ReadOnly = True
      Size = 100
    end
    object Daily_DB_DetilQTYIN: TFloatField
      FieldName = 'QTYIN'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
    object Daily_DB_DetilQTYOUT: TFloatField
      FieldName = 'QTYOUT'
      ReadOnly = True
      DisplayFormat = '#,##0.;(#,##0.)'
    end
  end
  object fcCube1: TfcCube
    DataSet = Daily_DB_Detil
    Options = [mdcoMakeDates, mdcoLoadAllFields, mdcoRefreshOnDatasetReopen]
    DefaultFormat.BooleanFormat.FormatStr = 'False,True'
    DefaultFormat.BooleanFormat.Kind = fkBoolean
    DefaultFormat.DateFormat.FormatStr = 'dd/MM/yyyy'
    DefaultFormat.DateFormat.Kind = fkDateTime
    DefaultFormat.TimeFormat.FormatStr = 'hh:mm'
    DefaultFormat.TimeFormat.Kind = fkDateTime
    DefaultFormat.DateTimeFormat.FormatStr = 'dd/MM/yyyy'
    DefaultFormat.DateTimeFormat.Kind = fkDateTime
    DefaultFormat.FloatFormat.FormatStr = '%2.n'
    DefaultFormat.FloatFormat.Kind = fkNumeric
    DefaultFormat.CurrencyFormat.FormatStr = '%2.2m'
    DefaultFormat.CurrencyFormat.Kind = fkNumeric
    DefaultFormat.IntegerFormat.FormatStr = '%2.n'
    DefaultFormat.IntegerFormat.Kind = fkNumeric
    DefaultFormat.PercentFormat.FormatStr = '%2.2n'
    DefaultFormat.PercentFormat.Kind = fkNumeric
    UseMultiLoad = False
    Hierarchies = <>
    Version = '1.10.3'
    Left = 512
    Top = 80
  end
  object fcSlice1: TfcSlice
    DefaultColWidth = 100
    Options = [mdsoNoTotalIfOneValue]
    Cube = fcCube1
    FieldsOrder = fcfloByFieldName
    ShowSplitFieldsInFieldList = fcssf_Default
    Left = 560
    Top = 80
  end
end
