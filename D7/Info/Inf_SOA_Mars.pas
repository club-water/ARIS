...
type
  TInf_SOA_Mars_Frm = class(TForm)
    ...
    UniQuery1: TUniQuery;
    Label1: TLabel;
    UniQuery1tanggal_minimum: TDateTimeField;
    UniQuery1tanggal_maximum: TDateTimeField;
...
var
  Inf_SOA_Mars_Frm: TInf_SOA_Mars_Frm;
implementation
...
procedure TInf_SOA_Mars_Frm.FormClose(Sender: TObject; var Action: TCloseAction);
var FileName :String;
begin
 ...
 if (not fcCube1.DataSet.IsEmpty) and (fcSlice1.HaveLayout) then
  ...
  Action := caFree;
  Inf_SOA_Mars_Frm := nil;
end;

procedure TInf_SOA_Mars_Frm.BitBtn1Click(Sender: TObject);
var AIndex: integer;
    Vmenit,Vdetik : integer;
    FileName :String;
begin
  cxGrid3DBTableView1.DataController.DataSource:=nil;
  FileName:=ExtractFileDir(Application.ExeName)+'\Inf_SOA_Mars_Frm.mds';
 if (not fcCube1.DataSet.IsEmpty) and (fcSlice1.HaveLayout) then
  begin
    fcSlice1.SaveToFile(FileName);
  end;
...
procedure TInf_SOA_Mars_Frm.BitBtn2Click(Sender: TObject);
begin
  close;
end;

procedure TInf_SOA_Mars_Frm.FormShow(Sender: TObject);
var i: Integer;
begin
UniQuery1.close;
UniQuery1.open;
Label1.caption:=datetostr(UniQuery1tanggal_minimum.AsDateTime)+' S.D '+datetostr(UniQuery1tanggal_maximum.AsDateTime);
...
procedure TInf_SOA_Mars_Frm.Button7Click(Sender: TObject);
var i:integer;
    vlist:string;
begin
  fcCube1.Active := false;
...
procedure TInf_SOA_Mars_Frm.Button1Click(Sender: TObject);
begin
  cxGrid3DBTableView1.ApplyBestFit;
end;

procedure TInf_SOA_Mars_Frm.cxGrid3DBTableView1CustomDrawCell(
  Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
var
  val : String;
...
procedure TInf_SOA_Mars_Frm.Button2Click(Sender: TObject);
begin
  cxGrid3DBTableView1.DataController.DataSource:=UniDataSource1;
end;

procedure TInf_SOA_Mars_Frm.Grid1Click(Sender: TObject);
begin
...
procedure TInf_SOA_Mars_Frm.DataRaw1Click(Sender: TObject);
begin
